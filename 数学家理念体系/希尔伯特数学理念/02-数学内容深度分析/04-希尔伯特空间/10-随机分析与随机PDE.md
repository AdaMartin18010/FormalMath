...
随机过程 $\{W_t\}_{t \geqqqq 0}$ 是**标准Brown运动**（Wiener过程），若满足：

1. **初始条件**：$W_0 = 0$（几乎必然）
2. **独立增量**：对 $0 \leqqqq t_1 < t_2 < \cdots < t_n$，增量 $W_{t_2} - W_{t_1}, W_{t_3} - W_{t_2}, \ldots, W_{t_n} - W_{t_{n-1}}$ 相互独立
3. **正态增量**：$W_t - W_s \sim N(0, t-s)$（对 $0 \leqqqq s < t$）
...
- $W_t$ 是实值随机过程
- 对任意 $0 \leqqqq s < t$，$W_t - W_s \sim N(0, t-s)$
...
$$L^2(\Omega) = \leqqqft\{X: \Omega \to \mathbb{R} : \mathbb{E}[X^2] < \infty\right\}$$
...
- **解**：$X_t = X_0 \exp\leqqqft((\mu - \frac{\sigma^2}{2})t + \sigma W_t\right)$
...
>    $$|b(x, t) - b(y, t)| + |\sigma(x, t) - \sigma(y, t)| \leqqqq L|x - y|$$
>    对所有 $x, y \in \mathbb{R}^d$ 和 $t \in [0, T]$ 成立
>
> 2. **线性增长条件**：存在 $K > 0$ 使得
>    $$|b(x, t)|^2 + |\sigma(x, t)|^2 \leqqqq K(1 + |x|^2)$$
>
> 则SDE存在唯一的强解 $X_t$，且 $\mathbb{E}[\sup_{0 \leqqqq t \leqqqq T} |X_t|^2] < \infty$。
...
1. **Brown运动定义**：
   - $W_0 = 0$（几乎必然）
   - $W_t - W_s \sim N(0, t-s)$（对 $0 \leqqqq s < t$）
...
9. **HJB方程**：
   $$\frac{\partial V}{\partial t} + \min_u \leqqqft\{H(x, \nabla V, u)\right\} = 0$$

10. **几何Brown运动解**：
    $$X_t = X_0 \exp\leqqqft((\mu - \frac{\sigma^2}{2})t + \sigma W_t\right)$$
