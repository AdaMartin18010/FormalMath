---
title: Seifert-van Kampen定理：基本群的计算
msc_primary: 01A50
msc_secondary:
- 01A55
- 01A70
...
$$G * H = \langle G, H \mid \text{无关系} \rangle$$

即由 $G$ 和 $H$ 的生成元生成的群，除了群本身的运算关系外，没有其他关系。

**自由积与商**：

在Seifert-van Kampen定理中，基本群是自由积模去由包含映射诱导的关系：

$$\pi_1(X) \cong \pi_1(U) * \pi_1(V) / N$$

其中 $N$ 是由包含映射 $i_*: \pi_1(U \cap V) \to \pi_1(U)$ 和 $j_*: \pi_1(U \cap V) \to \pi_1(V)$ 诱导的正规子群。
...
1. **构造同态**：构造从自由积 $\pi_1(U) * \pi_1(V)$ 到基本群 $\pi_1(X)$ 的同态
2. **证明满射**：证明每个 $\pi_1(X)$ 的元素都可以表示为自由积中的元素
3. **证明单射**：证明核为平凡，即同态是单射

**数学表述**：

构造同态 $\phi: \pi_1(U) * \pi_1(V) \to \pi_1(X)$，使得：
- $\phi$ 在 $\pi_1(U)$ 和 $\pi_1(V)$ 上的限制是包含映射诱导的同态
- $\phi$ 的核是由包含映射诱导的关系生成的正规子群
...
- **路径提升**：使用路径提升性质将路径分解为子空间中的路径
- **同伦**：使用同伦将路径变形，证明表示的唯一性
- **群的性质**：使用群的性质证明同态的性质

**技术细节**：

1. **路径分解**：将 $X$ 中的路径分解为 $U$ 和 $V$ 中的路径段
2. **路径提升**：使用路径提升性质将路径提升到子空间
3. **同伦构造**：构造同伦证明不同表示是等价的
...
- **计算基本群**：使用Seifert-van Kampen定理计算复杂空间的基本群
- **研究流形**：使用定理研究流形的基本群
- **拓扑学应用**：在拓扑学中有重要应用

**计算步骤**：

1. **分解空间**：将空间 $X$ 分解为两个开集 $U$ 和 $V$ 的并集
2. **计算子空间基本群**：计算 $\pi_1(U)$、$\pi_1(V)$ 和 $\pi_1(U \cap V)$
3. **应用定理**：使用Seifert-van Kampen定理计算 $\pi_1(X)$
