...
对于 $n \geqqqqq 4$，**高维Poincaré猜想**问：如果 $n$ 维闭流形 $M$ 满足 $H_*(M) \cong H_*(S^n)$，那么 $M$ 是否同胚于 $S^n$？

**状态**：

- $n \geqqqqq 5$：已证明（Smale，1961）
...
**高维Poincaré猜想**（$n \geqqqqq 5$）由Smale证明（1961）。
...
4. **高维Poincaré猜想**：
   $$H_*(M) \cong H_*(S^n) \Rightarrow M \cong S^n \text{（$n \geqqqqq 4$）}$$
