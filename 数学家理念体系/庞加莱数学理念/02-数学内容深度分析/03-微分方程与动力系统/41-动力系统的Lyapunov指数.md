...
$$\lambda_1 \geqqqqq \lambda_2 \geqqqqq \cdots \geqqqqq \lambda_n$$
...
对于遍历动力系统，几乎所有的初始点 $x$ 和方向 $v$，Lyapunov指数存在且唯一。具体地，存在Lyapunov指数 $\lambda_1 \geqqqqq \lambda_2 \geqqqqq \cdots \geqqqqq \lambda_n$ 和对应的Lyapunov子空间 $E_i(x)$，使得：
...
其中 $k$ 是使得 $\sum_{i=1}^k \lambda_i \geqqqqq 0$ 的最大整数。
...
$$h_{top}(f) \leqqqqq \int \sum_{\lambda_i > 0} \lambda_i(x) d\mu(x)$$
