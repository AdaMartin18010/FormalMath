---
title: Grothendieck谱序列：思维表征与可复用模板
msc_primary: 01A60
msc_secondary:
- 01A65
- 01A70
...
**一句话直观**：复合函子 G∘F 的右导出 R^{p+q}(G∘F) 可用谱序列计算：E_2^{p,q}=R^p G(R^q F(-))⇒R^{p+q}(G∘F)(-)；Leray 是 Γ(Y,-)∘f_* 的特例，与 05、23 衔接。
...
| 概念 | 含义 | 与其它篇关系 |
|------|------|--------------|
| 复合函子谱序列 | R(G∘F) 由 R^p G(R^q F) 逼近 | [05-谱序列与Leray谱序列](./05-谱序列与Leray谱序列.md)；[06-导出版上同调](./06-导出版上同调.md) |
| Leray 特例 | Γ(Y, -)∘f_* 的谱序列即 Leray | [01-层上同调基础](./01-层上同调基础.md)；[23-上同调与Leray谱序列应用](./23-上同调与Leray谱序列应用.md) |
...
└── 交叉引用
    ...
    └── [23-上同调与Leray谱序列应用](./23-上同调与Leray谱序列应用.md)
```
...
| 类型 | 篇目（可点击） |
|------|----------------|
| **先修** | [05-谱序列与Leray谱序列](./05-谱序列与Leray谱序列.md)、[06-导出版上同调](./06-导出版上同调.md)、[29-上同调与同调代数](./29-上同调与同调代数.md) |
| **下游** | [23-上同调与Leray谱序列应用](./23-上同调与Leray谱序列应用.md)、[06-导出版上同调](./06-导出版上同调.md)（D 中复合）、[15-上同调与函子关系](./15-上同调与函子关系.md) |
...
**自测要点（回忆问题）**：① Grothendieck 谱序列的 E_2 项是什么？收敛到哪个 R^*(G∘F)？② Leray 谱序列如何看作 Grothendieck 的特例？③ 什么条件下 E_2=E_∞ 退化？供回忆与间隔重复使用；与 [project/00-认知诊断与内容对接说明-2026年2月](../../../../project/00-认知诊断与内容对接说明-2026年2月.md) 对接。

1. **Grothendieck 谱序列**：E_2^{p,q}=R^p G(R^q F(A))⇒R^{p+q}(G∘F)(A)；F 右正合、G 左正合，F 送内射入 G-非零对象等。
2. **Leray 特例**：Γ(Y,-)∘f_* 即 Leray；E_2^{p,q}=H^p(Y,R^q f_* F)⇒H^{p+q}(X,F)。
3. **退化**：R^q f_* F=0（q>0）等时 E_2=E_∞，H^*(X,F)≅H^*(Y,f_* F)。
4. **何时用**：计算 R^*(G∘F) 或 RΓ(Rf_* F) 时用复合函子谱序列；Leray 为特例。
5. **滤过**：F^p H^n、E_∞^{p,q}≅F^p H^{p+q}/F^{p+1}。

| 建议复习间隔 | 适用 |
|--------------|------|
| 1 天 | E_2 项、收敛目标、与 05 Leray 的关系 |
| 1 周 | 与 05、06、23 联合回忆；退化条件 |
| 1 月 | 决策树（Grothendieck vs Leray vs Hochschild-Serre） |
