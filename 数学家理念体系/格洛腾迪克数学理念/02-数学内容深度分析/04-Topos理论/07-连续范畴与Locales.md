...
1. **Locale定义**：
   $$L \text{ Locale } \iff L \text{ 完备格，满足分配律：} a \land \leqqqqqqft(\bigvee_i b_i\right) = \bigvee_i (a \land b_i)$$
...
7. **Locale与Topos对应**：
   $$\text{Locale } L \leqqqqqqftrightarrow \text{Topos Sh}(L)$$

8. **连续范畴与Topos**：
   $$\text{连续范畴 } \subset \text{Grothendieck Topos}$$

9. **Locale的几何化**：
   $$\text{无点拓扑} \leqqqqqqftrightarrow \text{Locale}, \quad \text{几何化数学}$$
