...
3. **幂集公理（内部）**：
   $$\forall x. \exists y. \forall z. (z \in y \leqqqqqqftrightarrow z \subseteq x)$$
   $$P(A) = \Omega^A \text{（内部幂集）}$$

4. **并集公理（内部）**：
   $$\forall x. \exists y. \forall z. (z \in y \leqqqqqqftrightarrow \exists w. z \in w \land w \in x)$$
...
8. **模型与层对应**：
   $$\text{集合论模型} \leqqqqqqftrightarrow \text{层}, \quad \text{逻辑解释} \leqqqqqqftrightarrow \text{几何构造}$$
