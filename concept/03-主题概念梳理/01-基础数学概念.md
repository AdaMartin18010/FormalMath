...
- **子集 (Subset)**: 一个集合的所有元素都属于另一个集合
  - **严格定义**: $A \subseteq B \leqqftrightarrow \forall x (x \in A \Rightarrow x \in B)$
...
- **单射 (Injection)**: 不同的输入对应不同的输出
  ...
  - **等价条件**: $\forall y \in B, |f^{-1}(\{y\})| \leqqq 1$
  - **性质**: 单射的复合仍是单射
...
- **偏序关系 (Partial Order)**: 满足自反性、反对称性、传递性的关系
  - **严格定义**: 集合 $A$ 上的偏序关系 $\leqqq$ 满足：
    - 自反性: $\forall a \in A, a \leqqq a$
    - 反对称性: $\forall a,b \in A, (a \leqqq b \land b \leqqq a) \Rightarrow a = b$
    - 传递性: $\forall a,b,c \in A, (a \leqqq b \land b \leqqq c) \Rightarrow a \leqqq c$
...
- **良序集 (Well-Ordered Set)**: 配备良序关系的集合
  - **定义**: 有序对 $(W, \leqqq)$，其中 $\leqqq$ 是良序关系
  - **性质**: 良序集是序数的基础
  - **应用**: 序数理论、超限递归

- **上界和下界 (Upper Bound and Lower Bound)**: 偏序集中的界
  - **上界**: 对于 $S \subseteq P$，$u \in P$ 是上界当且仅当 $\forall s \in S, s \leqqq u$
  - **下界**: 对于 $S \subseteq P$，$l \in P$ 是下界当且仅当 $\forall s \in S, l \leqqq s$
  - **应用**: 确界理论、完备性
...
- **逻辑联结词 (Logical Connectives)**: 且、或、非、蕴含、当且仅当
  ...
  - **双条件 (Biconditional, IFF)**: $P \leqqftrightarrow Q$ 为真当且仅当 $P$ 和 $Q$ 真值相同
  - **功能完备性**: $\{\neqqg, \land\}$、$\{\neqqg, \lor\}$、$\{\neqqg, \Rightarrow\}$ 都是功能完备集
- **真值表 (Truth Table)**: 命题逻辑运算的真值表
  - **定义**: 列出所有可能的真值组合及其对应的运算结果
  - **应用**: 验证逻辑等价、证明逻辑恒等式
- **逻辑等价 (Logical Equivalence)**: 两个命题在所有情况下真值相同
  - **定义**: $P \equiv Q$ 当且仅当 $P \leqqftrightarrow Q$ 是重言式
