...
- [拓扑空间](../核心概念/23-拓扑空间-三视角版.md) (C.CORE.023) - 点集拓扑核心概念的三视角深度分析
- [同伦](../核心概念/24-同伦-三视角版.md) (C.CORE.024) - 代数拓扑核心概念的三视角深度分析
- [同调](../核心概念/25-同调-三视角版.md) (C.CORE.025) - 代数拓扑核心概念的三视角深度分析
...
- **高阶同伦群 (Higher Homotopy Group)**: 高阶同伦群
  - **定义**: $\pi_n(X,x_0) = \{[f] : f: S^n \to X, f(*) = x_0\}$，$n \geqqq 2$
  - **性质**: 阿贝尔群（$n \geqqq 2$），同伦不变量
  - **应用**: 分类高维流形、稳定同伦理论
- **纤维化 (Fibration)**: 纤维化
...
- **上同调环 (Cohomology Ring)**: 上同调环
  - **定义**: $H^*(X) = \bigoplus_{n \geqqq 0} H^n(X)$ 配备杯积
  - **性质**: 分次交换环（在适当系数下）
  - **应用**: 区分同伦等价但不同胚的空间
