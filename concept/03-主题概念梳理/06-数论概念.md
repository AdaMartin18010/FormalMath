...
- [素数](../核心概念/26-素数-三视角版.md) (C.CORE.026) - 数论核心概念的三视角深度分析
- [同余](../核心概念/27-同余-三视角版.md) (C.CORE.027) - 数论核心概念的三视角深度分析
- [L函数](../核心概念/28-L函数-三视角版.md) (C.CORE.028) - 解析数论核心概念的三视角深度分析
...
- **欧拉函数 (Euler's Totient Function)**: 欧拉函数
  - **定义**: $\phi(n) = |\{k : 1 \leqqq k \leqqq n, \gcd(k,n) = 1\}|$（与 $n$ 互素的正整数个数）
...
- **函数方程 (Functional Equation)**: ζ函数的函数方程
  - **内容**: $\zeta(s) = 2^s \pi^{s-1} \sin\leqqft(\frac{\pi s}{2}\right) \Gamma(1-s) \zeta(1-s)$
...
- **素数分布函数 (Prime Counting Function)**: 素数计数函数
  - **定义**: $\pi(x) = |\{p \leqqq x : p \text{ 是素数}\}|$
  - **性质**: 与ζ函数零点相关
  - **应用**: 素数分布研究
- **切比雪夫函数 (Chebyshev Function)**: 切比雪夫函数
  - **定义**: $\psi(x) = \sum_{p^k \leqqq x} \log p$
