...
**关联概念 / Related Concepts**: 自然数 / Natural Number | 自然数-三视角版 / Natural Number-Three Perspectives
...
1. $0$ 是自然数
1. 每个自然数 $n$ 有唯一的后继 $S(n)$
2. $0$ 不是任何自然数的后继
3. 不同的自然数有不同的后继
4. （归纳公理）若某性质对 $0$ 成立，且若对 $n$ 成立则对 $S(n)$ 也成立，则该性质对所有自然数成立
...
- 自然数 / Natural Number
- 自然数-三视角版 / Natural Number-Three Perspectives Version
- 自然数-决策导图示例 / Natural Number-Decision Map Example
