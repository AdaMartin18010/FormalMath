...
foreach ($file in $conceptFiles) {
    ...
    if ($titleLine) {
        $fullTitle = $titleLine -replace "^# ", ""
        $titleAnchor = ($fullTitle -replace "\([^)]+\)", "" -replace "[^\w\s-]", "" -replace "\s+", "-").ToLower()
    } else {
    ...
    foreach ($line in $lines) {
        ...
        if ($trimmed -match "^##\s+([0-9]+)\.\s+(.+)$") {
            $num = $matches[1]
            $text = $matches[2].Trim()
            $anchor = ($text -replace "\(编号:.*?\)", "" -replace "[^\w\s-]", "" -replace "\s+", "-").ToLower()
            $anchor = $anchor.Trim("-")
            if ($anchor) {
                $sections += @{
                    Level = 1
    ...
    foreach ($section in $sections) {
        if ($section.Level -eq 1) {
            ...
            $prevLevel1 = $section.Number
            $prevLevel2 = ""
        }
        elseif ($section.Level -eq 2) {
            $indent = "    "
            $newTOC += "$indent- [$($section.Number) $($section.Text)](#$($section.Anchor))`n"
            $prevLevel2 = $section.Number
        }
