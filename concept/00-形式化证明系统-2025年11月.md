...
1. 假设存在双射 $g: A \to \mathcal{P}(A)$ (反证法)
2. 定义 $B = \{x \in A : x \notin g(x)\}$ (对角线论证)
3. $B \in \mathcal{P}(A)$，因此存在 $b \in A$ 使得 $g(b) = B$ (双射性)
4. 若 $b \in B$，则 $b \notin g(b) = B$，矛盾 (步骤3, 4)
5. 若 $b \notin B$，则 $b \in f(b) = B$，矛盾 (步骤3, 4)
6. 因此不存在双射，$|A| < |\mathcal{P}(A)|$ (步骤2-6)
...
1. 由Peano公理5，$P = \mathbb{N}$ (归纳公理)
2. 因此 $[0 \in P \land \forall n (n \in P \Rightarrow S(n) \in P)] \Rightarrow P = \mathbb{N}$ (步骤1-2)
...
1. 构造Dedekind分割 $(A, B)$，其中 $A = \{q \in \mathbb{Q} : q < a_n \text{ 对充分大的 } n\}$ (构造)
2. $(A, B)$ 是Dedekind分割 (Cauchy性质)
3. 设 $L = (A, B)$ (定义)
4. $\lim_{n \to \infty} a_n = L$ (步骤2-4)
5. 因此 $\text{Cauchy}(a_n) \to \exists L \lim_{n \to \infty} a_n = L$ (步骤1-5)
...
1. 设 $G$ 是有限群，$H$ 是子群 (假设)
1. $G$ 可以分解为 $H$ 的左陪集的并：$G = \bigcup_{g \in G} gH$ (陪集分解)
2. 不同陪集不相交 (陪集性质)
3. 每个陪集的大小等于 $|H|$ (陪集性质)
4. $|G| = |G/H| \cdot |H|$ (步骤2-4)
5. 因此 $|H| \mid |G|$ (步骤5)
...
1. $F'(x) = \lim_{h \to 0} \frac{F(x+h) - F(x)}{h}$ (导数定义)
2. $= \lim_{h \to 0} \frac{1}{h} \int_x^{x+h} f(t) \, dt$ (积分性质)
3. 由积分中值定理，存在 $c \in [x, x+h]$ 使得 $\int_x^{x+h} f(t) \, dt = f(c)h$ (积分中值定理)
4. $= \lim_{h \to 0} f(c) = f(x)$ (连续性)
5. 因此 $F'(x) = f(x)$ (步骤2-5)
