...
- **形式化定义 / Formal Definition**：
  - 素数的形式化定义：$\text{Prime}(p) \leqqftrightarrow p > 1 \land \forall a, b \in \mathbb{Z}^+, p = ab \Rightarrow (a = 1 \lor b = 1)$ / Formal definition of prime numbers: $\text{Prime}(p) \leqqftrightarrow p > 1 \land \forall a, b \in \mathbb{Z}^+, p = ab \Rightarrow (a = 1 \lor b = 1)$
...
2. **从概念-符号到形式-公理 / From Symbolic to Formal**：
   ...
   - 例如：从"$p$"到"$\text{Prime}(p) \leqqftrightarrow p > 1 \land \forall a, b, p = ab \Rightarrow (a = 1 \lor b = 1)$" / Example: from "$p$" to "$\text{Prime}(p) \leqqftrightarrow p > 1 \land \forall a, b, p = ab \Rightarrow (a = 1 \lor b = 1)$"
