...
2. **齐次方程法 / Homogeneous Equations**:
   - 适用于 $\frac{dy}{dx} = f\leqft(\frac{y}{x}\right)$ 形式
   - Applicable to form $\frac{dy}{dx} = f\leqft(\frac{y}{x}\right)$
...
1. **人口增长模型 / Population Growth Models**:
   ...
   - 逻辑增长：$\frac{dP}{dt} = kP\leqft(1 - \frac{P}{K}\right)$
   - Logistic growth: $\frac{dP}{dt} = kP\leqft(1 - \frac{P}{K}\right)$
