...
# -*- coding: utf-8 -*-
"""
为缺少正文「参考」节的银层文档，根据 frontmatter 自动生成参考与延伸阅读节。

这能同时提升：
- 结构要素中的 references 覆盖率
- 正文精确引用标识符（ISBN/DOI/arXiv）密度

用法：
    python scripts/append_references_section.py
"""
...
def has_references_section(body: str):
    return bool(re.search(r"^#{1,6}\s+(?:参考|Reference|Bibliography|文献)", body, re.MULTILINE | re.IGNORECASE))
...
def process_file(path: Path):
    ...
    if fm is None or fm.get("level") != "silver":
        return False
    if has_references_section(body):
        return False
...
def main():
    ...
    for p in PROJECT_ROOT.rglob("*.md"):
        ...
        if process_file(p):
            changed += 1
    print(f"Appended references section to {changed} silver docs.")
