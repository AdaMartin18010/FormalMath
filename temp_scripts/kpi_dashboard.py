...
# KPI Dashboard for FormalMath Silver Layer
# Scans docs/00-银层核心课程/ and docs/03-分析学/01-实分析/定理证明/
...
theorem_pattern = re.compile(r'##?\s*定理\s*\d*\s*[（(]', re.IGNORECASE)
...
for base_path in silver_paths:
    ...
    for root, dirs, files in os.walk(base_path):
        for f in files:
            ...
            theorems = len(theorem_pattern.findall(content))
...
for course, data in sorted(kpi['courses'].items(), key=lambda x: -x[1]['docs']):
    ...
    th_target = docs * 3  # heuristic: 3 theorems per doc
    ex_target = docs * 10  # silver standard: 10 exercises per chapter
    print(f"    定理-证明覆盖率: {th}/{th_target} ({th/max(th_target,1)*100:.0f}%)")
