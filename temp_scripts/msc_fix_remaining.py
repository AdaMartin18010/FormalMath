...
# Remaining invalid patterns that can be auto-fixed
# XXAnn, XXAxx, XXNxx, XXUxx, XXVxx, XXExx -> XX
subcode_as_primary = re.compile(r'^([0-9]{2})([A-Z][0-9]{2,3})$')
fixed = 0
skipped = 0
...
for root, dirs, files in os.walk('G:/_src/FormalMath'):
    ...
    for f in files:
        ...
        sm = subcode_as_primary.match(primary)
        if not sm:
            skipped += 1
            continue
...
print(f"Remaining MSC fix: fixed={fixed}, skipped={skipped}")
