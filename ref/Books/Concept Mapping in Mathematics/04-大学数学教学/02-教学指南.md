...
**教学策略详细描述 / Detailed Teaching Strategy Description**:

1. **先行组织者策略 / Advance Organizer Strategy**: 在引入新概念前，使用概念映射展示上位概念与下位概念的关系，帮助学生建立认知锚点。例如，在教授「导数」前，先展示「函数→极限→导数」的概念层级。
2. **渐进分化策略 / Progressive Differentiation Strategy**: 从一般概念逐步细化到具体概念。如「函数」→「连续函数」→「可微函数」→「导数存在」。
3. **整合协调策略 / Integrative Reconciliation Strategy**: 通过跨链接显化不同主题间的关联，如导数与积分、微积分与线性代数的联系。
4. **社会建构策略 / Social Construction Strategy**: 组织同伴互评、小组协作构建概念映射，通过社会互动促进意义建构。
5. **反思修订策略 / Reflective Revision Strategy**: 定期要求学生根据反馈修订概念映射，培养元认知能力。

**主要效果 / Main Effects**:

1. **学习效果** - 提高学习效果 | Learning Effectiveness - Improves learning effectiveness
2. **理解深度** - 提高理解深度 | Understanding Depth - Improves understanding depth
3. **应用能力** - 提高应用能力 | Application Ability - Improves application ability
