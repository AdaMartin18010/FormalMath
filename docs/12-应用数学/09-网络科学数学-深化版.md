...
- 线性阈值模型：$v \text{激活} \leftrightarrow \sum_{u \in N(v)} w_{uv} \geq \theta_v$
...
- 通量平衡分析：$\max \mathbf{c}^T \mathbf{v}$，s.t. $S\mathbf{v} = 0, \mathbf{v}_{\min} \leq \mathbf{v} \leq \mathbf{v}_{\max}$
...
- 网络设计：$\min \sum_{e} c_e x_e$，s.t. $\sum_{e \in \delta(S)} x_e \geq 1, \forall S \subset V$
- 可靠性优化：$\max R(G) = P(\text{网络连通})$，s.t. $\sum_{e} c_e x_e \leq B$
...
- 网络流：$\max \sum_{i} P_i$，s.t. $\sum_{j} P_{ij} = P_i, |P_{ij}| \leq P_{\max,ij}$
