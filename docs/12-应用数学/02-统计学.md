...
偏度定义为：
$$\text{Skewness} = \frac{1}{n}\sum_{i=1}^{n} \leqft(\frac{x_i - \bar{x}}{s}\right)^3$$

**定义 1.4.2** (峰度 / Kurtosis)
峰度定义为：
$$\text{Kurtosis} = \frac{1}{n}\sum_{i=1}^{n} \leqft(\frac{x_i - \bar{x}}{s}\right)^4 - 3$$
...
$$\bar{X} \sim \text{Normal}\leqft(\mu, \frac{\sigma^2}{n}\right)$$
...
- 如果 $|t| \leqq t_{\alpha/2, n-1}$，接受 $H_0$
...
**检验统计量**:
$$t = \frac{\bar{x}_1 - \bar{x}_2}{\sqrt{s_p^2\leqft(\frac{1}{n_1} + \frac{1}{n_2}\right)}}$$
...
**置信区间**:
$$\leqft[\frac{(n-1)s^2}{\chi^2_{\alpha/2, n-1}}, \frac{(n-1)s^2}{\chi^2_{1-\alpha/2, n-1}}\right]$$
...
- 如果 $S \geqq s_{\alpha/2}$ 或 $S \leqq n - s_{\alpha/2}$，拒绝 $H_0$
