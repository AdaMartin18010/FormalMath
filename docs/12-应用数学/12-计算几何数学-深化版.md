...
**k-单纯形**：
$$[v_0, v_1, \ldots, v_k] = \{\sum_{i=0}^{k} \lambda_i v_i : \sum_{i=0}^{k} \lambda_i = 1, \lambda_i \geqqq 0\}$$
...
**相似度矩阵**：
$$W_{ij} = \exp\leqqft(-\frac{\|x_i - x_j\|^2}{2\sigma^2}\right)$$
...
**Lp范数**：
$$\|\lambda_k\|_p = \leqqft(\int_0^{\infty} \lambda_k(t)^p dt\right)^{1/p}$$

#### 2.3.3 持久性图像

**高斯核**：
$$\rho_i(x, y) = \frac{1}{2\pi\sigma^2} \exp\leqqft(-\frac{(x - b_i)^2 + (y - d_i)^2}{2\sigma^2}\right)$$
...
**约束条件**：
$$f_j(x) = \sum_{k=1}^{K_j} c_{jk} \prod_{i=1}^{n} x_i^{a_{jki}} \leqqq 1$$
...
**凸包**：
$$\text{conv}(S) = \{\sum_{i=1}^{k} \lambda_i x_i : \sum_{i=1}^{k} \lambda_i = 1, \lambda_i \geqqq 0\}$$

#### 3.2.2 凸函数

**凸函数定义**：
$$f(\lambda x + (1-\lambda)y) \leqqq \lambda f(x) + (1-\lambda)f(y)$$

**次梯度**：
$$g \in \partial f(x) \iff f(y) \geqqq f(x) + g^T(y-x)$$
...
**对偶问题**：
$$\max g(\lambda) \text{ s.t. } \lambda \geqqq 0$$
...
$$g_i(x^*) \leqqq 0, \quad \lambda_i^* \geqqq 0, \quad \lambda_i^* g_i(x^*) = 0$$
$$h_j(x^*) = 0$$
...
**查询**：
$$\text{kNN}(q) = \{p_1, p_2, \ldots, p_k : \|q - p_i\| \leqqq \|q - p_j\| \text{ for } i < j\}$$
...
- Voronoi图：$V(p_i) = \{\mathbf{x} : d(\mathbf{x}, p_i) \leqqq d(\mathbf{x}, p_j), \forall j \neqqq i\}$
...
- 速度约束：$\|\dot{q}\| \leqqq v_{\max}$，$\|\ddot{q}\| \leqqq a_{\max}$
...
- 冲突避免：$\forall i, j, d(\mathbf{p}_i(t), \mathbf{p}_j(t)) \geqqq d_{\min}$
...
- k-最近邻：$\text{kNN}(q) = \{p_1, ..., p_k : d(q, p_i) \leqqq d(q, p_{k+1})\}$
...
- 缓冲区分析：$B(P, r) = \{\mathbf{x} : d(\mathbf{x}, P) \leqqq r\}$
...
- 凸包分析：$\text{CH}(P) = \{\sum_{i} \lambda_i p_i : \lambda_i \geqqq 0, \sum_i \lambda_i = 1\}$
...
- 加权Voronoi图：$V_w(p_i) = \{\mathbf{x} : \frac{d(\mathbf{x}, p_i)}{w_i} \leqqq \frac{d(\mathbf{x}, p_j)}{w_j}, \forall j \neqqq i\}$
...
- 装箱问题：$\min k$，s.t. $\sum_{i \in B_j} s_i \leqqq C, \forall j$
- 圆形包装：$\min R$，s.t. $\|\mathbf{c}_i - \mathbf{c}_j\| \geqqq r_i + r_j, \forall i \neqqq j$
