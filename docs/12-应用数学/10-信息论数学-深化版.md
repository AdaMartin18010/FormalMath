...
- [信息论数学 - 深化版](#信息论数学---深化版)
  ...
  - [7. 🎯 应用案例 / Applications](#7--应用案例--applications)
    ...
    - [7.7 数据存储应用 / Data Storage Applications](#77-数据存储应用--data-storage-applications)
      - [7.7.1 分布式存储 / Distributed Storage](#771-分布式存储--distributed-storage)
...
- 率失真函数：$R(D) = \min_{p(\hat{x}|x): E[d(x,\hat{x})] \leq D} I(X; \hat{X})$
- JPEG量化：$Q(u,v) = \text{round}\left(\frac{F(u,v)}{Q_{\text{table}}(u,v)}\right)$
...
- 功率控制：$\min \sum_i P_i$ s.t. $\text{SINR}_i \geq \gamma$
...
- **应用场景**: 模型量化、剪枝、蒸馏
- **数学模型**:
  - 率失真理论：$R(D) = \min I(X; \hat{X})$ s.t. $E[d(X, \hat{X})] \leq D$
...
- **数学模型**:
  - 差分隐私：$P(\mathcal{M}(D) \in S) \leq e^\epsilon P(\mathcal{M}(D') \in S)$
  - 互信息上界：$I(X; \hat{X}) \leq \epsilon$
