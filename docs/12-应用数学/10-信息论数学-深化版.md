...
- 对称性：$I(X; Y) = I(Y; X)$
- 非负性：$I(X; Y) \geqq 0$
- 数据处理不等式：$I(X; Y) \geqq I(X; Z)$ if $X \to Y \to Z$
...
**Fano不等式**：
$$H(X|Y) \leqq H_b(P_e) + P_e \log(|\mathcal{X}| - 1)$$
...
容量区域：
$$R_1 \leqq \frac{1}{2} \log(1 + \frac{P_1}{N})$$
$$R_2 \leqq \frac{1}{2} \log(1 + \frac{P_2}{N})$$
$$R_1 + R_2 \leqq \frac{1}{2} \log(1 + \frac{P_1 + P_2}{N})$$
...
**率失真理论**：
$$R(D) = \min_{p(\hat{x}|x): E[d(X,\hat{X})] \leqq D} I(X; \hat{X})$$
...
**率失真优化**：
$$\min_{Q} D(Q) \text{ s.t. } R(Q) \leqq R_0$$
...
- 率失真函数：$R(D) = \min_{p(\hat{x}|x): E[d(x,\hat{x})] \leqq D} I(X; \hat{X})$
- JPEG量化：$Q(u,v) = \text{round}\leqft(\frac{F(u,v)}{Q_{\text{table}}(u,v)}\right)$
...
- 功率控制：$\min \sum_i P_i$ s.t. $\text{SINR}_i \geqq \gamma$
...
- **应用场景**: 模型量化、剪枝、蒸馏
- **数学模型**:
  - 率失真理论：$R(D) = \min I(X; \hat{X})$ s.t. $E[d(X, \hat{X})] \leqq D$
...
- **数学模型**:
  - 差分隐私：$P(\mathcal{M}(D) \in S) \leqq e^\epsilon P(\mathcal{M}(D') \in S)$
  - 互信息上界：$I(X; \hat{X}) \leqq \epsilon$
