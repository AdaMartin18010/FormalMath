...
1. **连续性**：$W(t)$ 是 $t$ 的连续函数
2. **独立增量**：对于 $0 \leqq s < t$，增量 $W(t) - W(s)$ 独立于 $W(s)$
...
```latex
S(t) = S(0) \exp\leqft(\leqft(\mu - \frac{\sigma^2}{2}\right)t + \sigma W(t)\right)
```

**数学分析**：

1. **对数正态分布**：
   - $\ln S(t) \sim N\leqft(\ln S(0) + \leqft(\mu - \frac{\sigma^2}{2}\right)t, \sigma^2 t\right)$
...
1. **对数变换**：
   - $f(x) = \ln x$
   - $d\ln S = \leqft(\mu - \frac{\sigma^2}{2}\right)dt + \sigma dW$
