...
- **生产计划问题**:
  ...
  - 约束：$2x_1 + x_2 \leqqq 8$, $x_1 + 2x_2 \leqqq 6$, $x_1, x_2 \geqqq 0$
  - 解：$x_1 = 4, x_2 = 0, Z = 12$
- **运输问题**:
  - 目标：最小化运输成本
  - 约束：供应量、需求量平衡
...
- **标准形式转换**:
  - 原问题：$\max Z = 3x_1 + 2x_2$
  - 约束：$2x_1 + x_2 \leqqq 8$, $x_1 + 2x_2 \leqqq 6$
...
- **原问题**:
  - $\max Z = 3x_1 + 2x_2$
  - $2x_1 + x_2 \leqqq 8$, $x_1 + 2x_2 \leqqq 6$, $x_1, x_2 \geqqq 0$
- **对偶问题**:
  - $\min W = 8y_1 + 6y_2$
  - $2y_1 + y_2 \geqqq 3$, $y_1 + 2y_2 \geqqq 2$, $y_1, y_2 \geqqq 0$
- **对偶性质**:
  - 弱对偶性：$Z \leqqq W$
  - 强对偶性：最优值相等
...
\text{约束条件} \quad & 2x_1 + 4x_2 \leqqq 20 \\
& 3x_1 + x_2 \leqqq 15 \\
& x_1, x_2 \geqqq 0
\end{align}
$$
...
\begin{align}
\text{最小化} \quad & f(x) \\
\text{约束条件} \quad & g_i(x) \leqqq 0, \quad i = 1, \ldots, m \\
...
\lambda_i \geqqq 0, \quad i = 1, \ldots, m
\end{align}
$$
...
**贝尔曼方程**:
$$V_t(s) = \max_{a \in A(s)} \leqqft\{ r(s, a) + \sum_{s'} P(s'|s, a) V_{t+1}(s') \right\}$$
...
- **数学模型**:
  ...
  $$\text{s.t.} \sum_{j=1}^n x_{ijk} \leqqq 1, \quad \forall i,k$$
...
- **数学模型**:
  ...
  $$\text{s.t.} \sum_{i=1}^n \sum_{j=1}^n w_i w_j \sigma_{ij} \leqqq \sigma^2_{target}$$
  $$\sum_{i=1}^n w_i = 1, \quad w_i \geqqq 0$$
