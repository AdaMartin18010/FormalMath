...
- [线性代数与矩阵理论 - 国际标准深度扩展版](#线性代数与矩阵理论---国际标准深度扩展版)
  ...
  - [概述](#概述)
  - [🗺️ 线性代数核心概念思维导图](#线性代数核心概念思维导图)
  - [📊 线性代数核心概念多维知识矩阵](#线性代数核心概念多维知识矩阵)
  - [一、向量空间理论](#一向量空间理论)
  ...
  - [附录：国际标准证明提纲与练习清单](#附录国际标准证明提纲与练习清单)
    ...
    - [B. 四大基本子空间（Four Fundamental Subspaces）](#b-四大基本子空间four-fundamental-subspaces)
      - [正交性关系（Orthogonality Relations）](#正交性关系orthogonality-relations)
      - [四个基本子空间与线性方程组](#四个基本子空间与线性方程组)
      - [奇异值分解（SVD）视角](#奇异值分解svd视角)
  ...
  - [附录：MIT 术语对照表 / MIT Terminology Cross-Reference](#附录mit-术语对照表-mit-terminology-cross-reference)
...
|---------|---------|---------|---------|---------|---------|
| 向量空间 | 向量空间 | 公理化定义 | 线性结构 | ℝⁿ, ℂⁿ | 数学基础 |
| 向量空间 | 基 | 线性无关生成集 | 基唯一性 | 标准基 | 向量表示 |
| 向量空间 | 维数 | 基的向量个数 | 维数唯一性 | dim(ℝⁿ) = n | 向量空间分类 |
| 矩阵理论 | 矩阵 | 数表 | 矩阵运算 | m×n矩阵 | 线性变换 |
| 矩阵理论 | 行列式 | 矩阵函数 | 可逆性判定 | det(A) | 线性方程组 |
| 矩阵理论 | 秩 | 行/列向量组秩 | 秩唯一性 | rank(A) | 线性方程组 |
| 矩阵理论 | SVD分解 | 奇异值分解 | 存在性 | A = UΣV* | 数据降维 |
| 线性变换 | 线性变换 | 保持线性运算 | 矩阵表示 | T: V→W | 线性映射 |
| 线性变换 | 核与像 | 零空间与值域 | 秩-零化度定理 | ker(T), im(T) | 线性方程组 |
| 特征值理论 | 特征值 | 特征方程根 | 特征值性质 | λ, v | 对角化 |
| 特征值理论 | 对角化 | 相似对角矩阵 | 可对角化条件 | P⁻¹AP = D | 矩阵简化 |
| 特征值理论 | Jordan标准形 | 准对角矩阵 | 存在性 | Jordan形 | 矩阵分类 |
| 内积空间 | 内积空间 | 内积定义 | 正交性 | (·,·) | 几何结构 |
| 内积空间 | 正交基 | 正交向量基 | 标准正交基 | 正交基 | 函数展开 |
| 应用 | 主成分分析 | 数据降维 | SVD | PCA | 机器学习 |
| 应用 | 线性回归 | 最小二乘 | 法方程 | 线性回归 | 数据分析 |
...
### B. 四大基本子空间（Four Fundamental Subspaces）

**MIT 18.06 Linear Algebra 框架**（Gilbert Strang）：

对于矩阵 $A \in \mathbb{R}^{m \times n}$（秩为 $r$），四个基本子空间构成线性代数的核心框架：
...
#### 正交性关系（Orthogonality Relations）

```

ℝⁿ:  Row(A) = C(Aᵀ)  ⟂  N(A)     (维数: r + (n-r) = n)
          ↑            ↑
       A的行空间      A的零空间

ℝᵐ:  Col(A) = C(A)   ⟂  N(Aᵀ)    (维数: r + (m-r) = m)
          ↑            ↑
       A的列空间      A的左零空间
...
- **维数公式**：$\dim C(A) + \dim N(A) = n$，$\dim C(A^T) + \dim N(A^T) = m$
- **秩的唯一性**：$\text{rank}(A) = \dim C(A) = \dim C(A^T) = r$

#### 四个基本子空间与线性方程组

对于方程组 $Ax = b$：

- **有解条件**：$b \in C(A)$（$b$ 必须在 $A$ 的列空间中）
- **唯一性**：若 $x_p$ 是一个特解，则通解为 $x = x_p + x_n$，其中 $x_n \in N(A)$
- **最小二乘残差**：$r = b - A\hat{x} \in N(A^T)$，且与 $C(A)$ 正交

#### 奇异值分解（SVD）视角

$A = U\Sigma V^T$ 揭示了四个子空间的正交基：

- $C(A)$：由 $U$ 的前 $r$ 列张成
- $N(A^T)$：由 $U$ 的后 $m-r$ 列张成
- $C(A^T)$：由 $V$ 的前 $r$ 列张成
- $N(A)$：由 $V$ 的后 $n-r$ 列张成

> **参考**：MIT 18.06 Linear Algebra, Gilbert Strang, "The Four Fundamental Subspaces"
...
- [交互式图表增强（线性变换/特征值可视化）](./../../../01-基础数学/集合论/集合论交互式内容增强-2025年1月.md)
- [定理证明补充（秩-零化度定理、谱定理）](./../../../01-基础数学/集合论/基数序数理论证明深化-2025年1月.md)
- [反例与特殊情况补充（线性无关/基/矩阵反例）](./../../../反例与特殊情况补充-2025年1月.md)
- [历史背景补充（线性代数与矩阵理论发展）](./../../../历史背景补充-2025年1月.md)

---

## 附录：MIT 术语对照表 / MIT Terminology Cross-Reference

本文档内容与 MIT 18.06 Linear Algebra（Gilbert Strang 教授）课程保持对齐。以下是术语对照：

| MIT 术语（英文） | 本文档术语（中文） | 符号/备注 | 对齐状态 |
|-----------------|-------------------|-----------|----------|
| Column Space | 列空间 | $C(A)$ | ✅ 完全一致 |
| Nullspace | 零空间 | $N(A)$ | ✅ 完全一致 |
| Row Space | 行空间 | $C(A^T)$ 或 $\text{Row}(A)$ | ✅ 完全一致 |
| Left Nullspace | 左零空间 | $N(A^T)$ | ✅ 已添加 |
| Four Fundamental Subspaces | 四个基本子空间 | 见附录 B | ✅ 已添加框架 |
| Rank | 秩 | $\text{rank}(A)$ | ✅ 完全一致 |
| Pivot | 主元 | 高斯消元中的非零首项 | ✅ 一致 |
| Free Variable | 自由变量 | 对应于非主元列 | ✅ 一致 |
| Reduced Row Echelon Form (RREF) | 行最简形 | 见附录 A | ✅ 一致 |
| LU Decomposition | LU 分解 | $A = LU$ | ✅ 完全一致 |
| Singular Value Decomposition (SVD) | 奇异值分解 | $A = U\Sigma V^T$ | ✅ 完全一致 |
| Orthogonal Matrix | 正交矩阵 | $Q^T Q = I$ | ✅ 完全一致 |
| Projection Matrix | 投影矩阵 | $P = A(A^TA)^{-1}A^T$ | ✅ 完全一致 |
| Least Squares | 最小二乘 | $\min \|Ax - b\|^2$ | ✅ 完全一致 |

> **参考来源**：MIT OpenCourseWare, 18.06 Linear Algebra, Spring 2010, Gilbert Strang
