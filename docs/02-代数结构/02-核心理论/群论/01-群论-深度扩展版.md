...
> **完整证明（含三个Sylow定理）参见**: [群论重要定理完整证明补充 - Sylow定理](04-群论重要定理完整证明补充.md#二定理-sylow定理)
...
> **完整证明参见**: [群论重要定理完整证明补充 - 马施克定理](04-群论重要定理完整证明补充.md#一定理-马施克定理-maschkes-theorem)
...
- [域论](./03-群论-深度扩展版.md)
- [模论](./../../../04-几何与拓扑/03-拓扑学内容/04-同伦论-深度扩展版.md)
...
本章节建立群论内容与Mathlib4的对应关系。

#### 基础群论模块

| 概念 | Mathlib4模块 | 定义/定理 |
|------|-------------|-----------|
| 群 | `Mathlib.Algebra.Group.Basic` | `Group`, `CommGroup` |
| 子群 | `Mathlib.Algebra.Group.Subgroup` | `Subgroup` |
| 同态 | `Mathlib.Algebra.Group.Hom` | `MonoidHom` |
| 商群 | `Mathlib.GroupTheory.QuotientGroup` | `QuotientGroup` |
| 陪集 | `Mathlib.GroupTheory.Coset` | `leftCoset`, `rightCoset` |

#### 核心定理对应

| 定理名称 | 本项目位置 | Mathlib4位置 |
|----------|-----------|--------------|
| **拉格朗日定理** | 本节第6.2.1节 | `Subgroup.index_mul_card` |
| **第一同构定理** | 本节第6.2.2节 | `MonoidHom.quotientKerEquivRange` |
| **西罗第一定理** | 本节第6.2.3节 | `Sylow.exists_subgroup_card_pow_prime` |
| **西罗第二定理** | 本节第6.2.3节 | `Sylow.isConj` |
| **西罗第三定理** | 本节第6.2.3节 | `Sylow.card_sylow_modEq_one` |

### 定义对应表

**群 (Group)**

- 本项目：`class Group (G : Type*)` - 群的抽象定义
- Mathlib4：`class Group (G : Type u)` - 带幺半群的逆运算扩展

**子群 (Subgroup)**

- 本项目：`structure Subgroup (G : Type*) [Group G]`
- Mathlib4：`structure Subgroup (G : Type u) [Group G]`

**群同态 (Group Homomorphism)**

- 本项目：`structure GroupHom (G H : Type*) [Group G] [Group H]`
- Mathlib4：`structure MonoidHom (M : Type*) (N : Type*)` 和 `GroupHomClass`

### 形式化代码链接

- **拉格朗日定理**: [Lean4代码](../../../09-形式化证明/Lean4/LagrangeTheorem.lean)
- **第一同构定理**: [Lean4代码](../../../09-形式化证明/Lean4/FirstIsomorphismTheorem.lean)
...
1. **命名差异**: 本项目使用`GroupHom`，Mathlib4使用`MonoidHom`并配合`GroupHomClass`
2. **结构层次**: Mathlib4有更精细的类型类层次（如`DivInvMonoid`等中间类型类）
3. **证明风格**: Mathlib4大量使用`simp`和自动化，本项目展示更多显式步骤

### 学习路径建议

1. **入门**: 阅读`Mathlib.Algebra.Group.Defs`了解基本类型类层次
2. **进阶**: 学习`Mathlib.GroupTheory.Subgroup`和`Mathlib.GroupTheory.QuotientGroup`
3. **深入**: 研究`Mathlib.GroupTheory.Sylow`中的西罗定理形式化
4. **实践**: 对比本项目Lean4代码与Mathlib4源码，理解证明技巧

---

**最后更新**: 2026年4月3日
**Mathlib4版本**: 4.19.0
**对齐负责人**: FormalMath项目团队
