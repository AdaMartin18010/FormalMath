...
theorem noether_theorem {G : Type*} [Group G] [LieGroup G]
  ...
  -- 3. 诺特流的构造
  -- 这里给出构造性定义
  use fun x => ∂[g] (lagrangian (action g x))
  -- 守恒量的验证需要证明散度为零
  sorry  -- 完整证明涉及微分几何和变分法
...
theorem selection_rule {G : Type*} [Group G]
  ...
  -- 选择定则表明：跃迁允许当且仅当直积表示包含平凡表示
  -- 这是维格纳-埃卡特定理的直接推论
  sorry  -- 完整证明需要Clebsch-Gordan系数计算
...
2. GAP - Groups, Algorithms, and Programming. <https://www.gap-system.org/[需更新[需更新]]>
