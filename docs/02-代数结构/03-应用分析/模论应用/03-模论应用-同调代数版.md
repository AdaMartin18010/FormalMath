...
2. $\operatorname{pd}_R(M) \leqqq n$ 当且仅当 $\operatorname{Ext}_R^{n+1}(M, N) = 0$ 对所有模 $N$
3. 投射维数在短正合列中满足不等式
...
2. $\operatorname{id}_R(M) \leqqq n$ 当且仅当 $\operatorname{Ext}_R^{n+1}(N, M) = 0$ 对所有模 $N$
...
2. $\operatorname{fd}_R(M) \leqqq n$ 当且仅当 $\operatorname{Tor}_R^{n+1}(M, N) = 0$ 对所有模 $N$
