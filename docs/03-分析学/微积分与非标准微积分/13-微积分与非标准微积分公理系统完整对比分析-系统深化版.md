...
\begin{align}
& \text{自反性}: a \leqq a \\
& \text{反对称性}: a \leqq b \land b \leqq a \implies a = b \\
& \text{传递性}: a \leqq b \land b \leqq c \implies a \leqq c \\
...
\begin{align}
& \text{内部自反性}: a \leqq a \\
& \text{内部反对称性}: a \leqq b \land b \leqq a \implies a = b \\
& \text{内部传递性}: a \leqq b \land b \leqq c \implies a \leqq c \\
...
```latex
\text{标准部分可微性}: f \text{在} a \text{可微} \iff \text{st}\leqft(\frac{f(a+\epsilon) - f(a)}{\epsilon}\right) \text{存在}
```
...
```latex
\text{等价性}: \phi \leqftrightarrow \psi \iff (\phi \to \psi) \land (\psi \to \phi)
```
...
```latex
\text{内部等价性}: \phi \leqftrightarrow^* \psi \iff (\phi \to \psi)^* \land (\psi \to \phi)^*
```
...
\text{标准阿基米德性} \leqftrightarrow \text{非标准非阿基米德性}
\text{标准有序性} \leqftrightarrow \text{非标准内部有序性}
```
...
```latex
\text{标准量词} \leqftrightarrow \text{非标准内部量词}
\text{标准推理} \leqftrightarrow \text{非标准内部推理}
\text{标准等价} \leqftrightarrow \text{非标准内部等价}
```
...
```latex
\text{内部速度公理}: v(t) = \text{st}\leqft(\frac{\Delta x}{\Delta t}\right)
\text{内部加速度公理}: a(t) = \text{st}\leqft(\frac{\Delta v}{\Delta t}\right)
\text{内部功公理}: W = \text{st}\leqft(\int_a^b F(x)dx\right)
```
...
```latex
\text{内部电场公理}: \mathbf{E} = -\text{st}\leqft(\frac{\Delta \phi}{\Delta t}\right)
\text{内部磁场公理}: \mathbf{B} = \text{st}\leqft(\frac{\Delta A}{\Delta t}\right)
\text{内部电磁波公理}: \nabla \times \mathbf{E} = -\text{st}\leqft(\frac{\Delta \mathbf{B}}{\Delta t}\right)
```
...
```latex
\text{内部状态方程公理}: \frac{dx}{dt} = \text{st}\leqft(\frac{x(t+\epsilon) - x(t)}{\epsilon}\right)
\text{内部传递函数公理}: G(s) = \text{st}\leqft(\int_0^N g(\tau)e^{-s\tau}d\tau\right)
...
```latex
\text{内部傅里叶变换公理}: X(f) = \text{st}\leqft(\int_{-N}^{N} x(t)e^{-j2\pi ft}dt\right)
\text{内部卷积公理}: y(t) = \text{st}\leqft(\int_{-N}^{N} x(\tau)h(t-\tau)d\tau\right)
\text{内部采样公理}: x_s(t) = \text{st}\leqft(\sum_{n=-M}^{M} x(nT)\delta(t-nT)\right)
```
