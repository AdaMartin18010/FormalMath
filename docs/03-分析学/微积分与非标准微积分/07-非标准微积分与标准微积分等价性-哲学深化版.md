...
```latex
f'(a) = \text{st}\leqft(\frac{f(a + \epsilon) - f(a)}{\epsilon}\right)
```
...
```latex
\int_a^b f(x) dx = \text{st}\leqft(\sum_{i=0}^{N-1} f(x_i) \Delta x\right)
```
...
```latex
\forall X \neqq \emptyset, \forall f : X \to \mathcal{P}(X) \setminus \{\emptyset\}, \exists g : X \to \bigcup X, \forall x \in X, g(x) \in f(x)
```
