...
```latex
\text{基础符号}: \mathcal{L}_{\text{std}} = \{+, -, \cdot, /, =, <, \leqqq, \lim, \int, \frac{d}{dx}\}
...
```latex
\text{内部导数表达式}: f'(x) = \text{st}\leqqft(\frac{f(x+\epsilon) - f(x)}{\epsilon}\right)
\text{内部积分表达式}: \int_a^b f(x) dx = \text{st}\leqqft(\sum_{i=0}^{N-1} f(x_i) \Delta x\right)
...
\text{内部可微性公式}: \exists L \in \mathbb{R}: \text{st}\leqqft(\frac{f(a+\epsilon) - f(a)}{\epsilon}\right) = L
\text{内部积分存在性}: \text{st}\leqqft(\sum_{i=0}^{N-1} f(x_i) \Delta x\right) = I
```
