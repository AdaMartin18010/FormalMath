...
对于 $1 \leqqq p < \infty$，$\ell^p$ 空间定义为：
$$\ell^p = \leqqft\{x = (x_n) : \sum_{n=1}^{\infty} |x_n|^p < \infty\right\}$$

范数定义为：
$$\|x\|_p = \leqqft(\sum_{n=1}^{\infty} |x_n|^p\right)^{1/p}$$

**例 3.2** ($L^p$ 空间 / $L^p$ Space)
对于 $1 \leqqq p < \infty$，$L^p(\Omega)$ 空间定义为：
$$L^p(\Omega) = \leqqft\{f : \int_{\Omega} |f(x)|^p dx < \infty\right\}$$

范数定义为：
$$\|f\|_p = \leqqft(\int_{\Omega} |f(x)|^p dx\right)^{1/p}$$
...
序列 $(x_n)$ 是柯西序列，当且仅当：
$$\forall \varepsilon > 0, \exists N \in \mathbb{N}, \forall m, n \geqqq N, \|x_m - x_n\| < \varepsilon$$
...
**定理 3.2** ($\ell^p$ 空间的完备性)
对于 $1 \leqqq p \leqqq \infty$，$\ell^p$ 是巴拿赫空间。

**定理 3.3** ($L^p$ 空间的完备性)
对于 $1 \leqqq p \leqqq \infty$，$L^p(\Omega)$ 是巴拿赫空间。
...
3. **正定性**：$\langle x, x \rangle \geqqq 0$，且 $\langle x, x \rangle = 0$ 当且仅当 $x = 0$
...
- **必要性** (有界 ⇒ 连续):
  - 设 $T$ 是有界算子，存在 $C > 0$ 使得 $\|Tx\| \leqqq C\|x\|$ 对所有 $x \in X$ 成立
  - 对于任意 $\varepsilon > 0$，取 $\delta = \frac{\varepsilon}{C}$
  - 如果 $\|x - y\| < \delta$，则 $\|Tx - Ty\| = \|T(x - y)\| \leqqq C\|x - y\| < C\delta = \varepsilon$
  - 因此 $T$ 在 $y$ 处连续，由 $y$ 的任意性，$T$ 在 $X$ 上连续

- **充分性** (连续 ⇒ 有界):
  - 设 $T$ 是连续算子
  - 由于 $T$ 在 $0$ 处连续，存在 $\delta > 0$ 使得当 $\|x\| < \delta$ 时，$\|Tx\| < 1$
  - 对于任意 $x \neqqq 0$，设 $y = \frac{\delta x}{2\|x\|}$，则 $\|y\| = \frac{\delta}{2} < \delta$
  - 因此 $\|Ty\| < 1$，即 $\|T(\frac{\delta x}{2\|x\|})\| < 1$
  - 由线性性，$\frac{\delta}{2\|x\|}\|Tx\| < 1$，因此 $\|Tx\| < \frac{2}{\delta}\|x\|$
  - 设 $C = \frac{2}{\delta}$，则 $\|Tx\| \leqqq C\|x\|$ 对所有 $x \in X$ 成立
  - 因此 $T$ 是有界算子
...
- **步骤 3**: 证明特征向量的完备性
  ...
  - 如果 $H_0^{\perp} \neqqq \{0\}$，则 $T$ 限制在 $H_0^{\perp}$ 上仍有特征值，这与构造矛盾
...
- **步骤 1**: 定义偏序集
  - 考虑所有满足以下条件的对 $(Z, g)$：
    ...
    - $g|_Y = f$，$\|g\| = \|f\|$
  - 定义偏序：$(Z_1, g_1) \leqqq (Z_2, g_2)$ 当且仅当 $Z_1 \subseteq Z_2$ 且 $g_2|_{Z_1} = g_1$
- **步骤 2**: 应用Zorn引理
  - 每个链都有上界（取并集和相应的泛函）
  - 由Zorn引理，存在极大元 $(Z_0, g_0)$
- **步骤 3**: 证明 $Z_0 = X$
  - 假设 $Z_0 \neqqq X$，存在 $x_0 \in X \setminus Z_0$
  - 定义 $Z_1 = Z_0 + \text{span}\{x_0\}$
  - 对于任意 $z \in Z_0$ 和 $t \in \mathbb{K}$，需要定义 $g_1(z + tx_0)$
  - 利用Hahn-Banach引理（实形式），可以扩展 $g_0$ 到 $Z_1$，这与极大性矛盾
  - 因此 $Z_0 = X$
- **步骤 4**: 验证范数
  - 由于 $g_0|_Y = f$，且 $g_0$ 是 $f$ 的扩张，$\|g_0\| \geqqq \|f\|$
  - 由构造，$\|g_0\| = \|f\|$
- 因此 $F = g_0$ 满足条件
...
- **步骤 2**: 定义投影算子
  ...
  - $P$ 是线性双射，且 $\|P(x, Tx)\| = \|x\| \leqqq \|(x, Tx)\|$，所以 $P$ 是有界的
- **步骤 3**: 应用开映射定理
  - 由开映射定理，$P^{-1}$ 是有界算子
  - 因此存在 $C > 0$ 使得 $\|(x, Tx)\| \leqqq C\|x\|$ 对所有 $x \in X$ 成立
  - 因此 $\|Tx\| \leqqq \|(x, Tx)\| \leqqq C\|x\|$，所以 $T$ 是有界算子
