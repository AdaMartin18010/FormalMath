...
2. **序公理** (Order Axioms)：
   - 自反性：$a \leqqq a$
   - 反对称性：$a \leqqq b \wedge b \leqqq a \Rightarrow a = b$
   - 传递性：$a \leqqq b \wedge b \leqqq c \Rightarrow a \leqqq c$
...
$$\forall \epsilon > 0, \exists N \in \mathbb{N}, \forall n \geqqq N, |a_n - L| < \epsilon$$
...
- 设 $N = \max\{N_1, N_2\}$，则当 $n > N$ 时：
  $$|L_1 - L_2| = |L_1 - a_n + a_n - L_2| \leqqq |a_n - L_1| + |a_n - L_2| < 2\varepsilon = |L_1 - L_2|$$
- 这矛盾，因此 $L_1 = L_2$，极限唯一
...
- 设 $N = \max\{N_1, N_2\}$，则当 $n > N$ 时：
  $$|(a_n + b_n) - (A + B)| = |(a_n - A) + (b_n - B)| \leqqq |a_n - A| + |b_n - B| < \varepsilon$$
...
- 设 $N = \max\{N_1, N_2\}$，则当 $n > N$ 时：
  $$|a_n b_n - AB| = |a_n(b_n - B) + B(a_n - A)| \leqqq |a_n||b_n - B| + |B||a_n - A| < M \cdot \frac{\varepsilon}{2M} + |B| \cdot \frac{\varepsilon}{2(|B| + 1)} < \varepsilon$$
...
- 设 $N = \max\{N_0, N_1\}$，则当 $n > N$ 时：
  $$\leqqft|\frac{1}{b_n} - \frac{1}{B}\right| = \frac{|b_n - B|}{|b_n||B|} < \frac{2|b_n - B|}{|B|^2} < \varepsilon$$
...
- **情况 1**: 假设 $(a_n)$ 是单调递增且有上界的序列
  ...
  - 对于任意 $\varepsilon > 0$，由于 $L$ 是上确界，存在 $N \in \mathbb{N}$ 使得 $L - \varepsilon < a_N \leqqq L$
  - 由于序列单调递增，对于所有 $n \geqqq N$，有 $a_N \leqqq a_n \leqqq L$
  - 因此 $L - \varepsilon < a_n \leqqq L$，即 $|a_n - L| < \varepsilon$
  - 所以 $\lim_{n \rightarrow \infty} a_n = L$

- **情况 2**: 假设 $(a_n)$ 是单调递减且有下界的序列
  - 类似地，集合 $A = \{a_n : n \in \mathbb{N}\}$ 有下确界 $L = \inf A$
  - 对于任意 $\varepsilon > 0$，存在 $N \in \mathbb{N}$ 使得 $L \leqqq a_N < L + \varepsilon$
  - 由于序列单调递减，对于所有 $n \geqqq N$，有 $L \leqqq a_n \leqqq a_N$
  - 因此 $L \leqqq a_n < L + \varepsilon$，即 $|a_n - L| < \varepsilon$
  - 所以 $\lim_{n \rightarrow \infty} a_n = L$
...
$$\lim_{x \rightarrow a^+} f(x) = L \leqqftrightarrow \forall \epsilon > 0, \exists \delta > 0, \forall x \in A, a < x < a + \delta \Rightarrow |f(x) - L| < \epsilon$$

**定义 3.1.8** (左极限 / Left Limit)
$$\lim_{x \rightarrow a^-} f(x) = L \leqqftrightarrow \forall \epsilon > 0, \exists \delta > 0, \forall x \in A, a - \delta < x < a \Rightarrow |f(x) - L| < \epsilon$$
...
其中 $\|P\| = \max_{1 \leqqq i \leqqq n} (x_i - x_{i-1})$ 是分割的范数。
...
- **必要性**: 如果 $f$ 可积，设 $\int_a^b f(x) dx = I$
  ...
  - 可以证明 $L(f, P) \leqqq S(f, P) \leqqq U(f, P)$ 对所有黎曼和成立
