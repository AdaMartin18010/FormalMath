...
因此：
$$|a_n - a_m| \leqqq |a_n - L| + |a_m - L| < \varepsilon$$
...
$$|a_n - L| \leqqq |a_n - a_{n_k}| + |a_{n_k} - L| < \varepsilon$$
...
**定理 1.3** (勒贝格控制收敛定理)
设 $\{f_n\}$ 是可测函数序列，$|f_n| \leqqq g$，其中 $g$ 是可积函数，且 $f_n \to f$ 几乎处处，则：
...
**步骤1**：应用法图引理
由于 $f_n \to f$ 几乎处处，且 $|f_n| \leqqq g$，可以应用法图引理。
