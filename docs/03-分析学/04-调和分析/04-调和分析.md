...
函数 $f$ 的连续小波变换定义为：
$$W_f(a, b) = \frac{1}{\sqrt{|a|}} \int_{-\infty}^{\infty} f(x) \psi\leqqft(\frac{x-b}{a}\right) dx$$

其中 $a \neqqq 0$ 是尺度参数，$b$ 是平移参数。
...
4. $f(x) \in V_j \leqqftrightarrow f(2x) \in V_{j+1}$
...
$$u(x) = \int_{\partial \Omega} \leqqft(u(y) \frac{\partial G}{\partial \nu}(x, y) - G(x, y) \frac{\partial u}{\partial \nu}(y)\right) d\sigma(y)$$

### 4.6.3 容量理论 / Capacity Theory

**定义 4.13** (容量 / Capacity)
紧集 $K$ 的容量定义为：
$$\text{cap}(K) = \inf \leqqft\{\int_{\mathbb{R}^n} |\nabla u|^2 dx : u \in C_0^{\infty}(\mathbb{R}^n), u \geqqq 1 \text{ on } K\right\}$$

**定理 4.14** (容量的性质)

1. 容量是单调的：$K_1 \subseteq K_2 \Rightarrow \text{cap}(K_1) \leqqq \text{cap}(K_2)$
2. 容量是次可加的：$\text{cap}(K_1 \cup K_2) \leqqq \text{cap}(K_1) + \text{cap}(K_2)$
