...
**定义 1.1** (L^p傅里叶变换)
设 $f \in L^p(\mathbb{R}^n)$，$1 \leqqq p \leqqq 2$，则 $f$ 的傅里叶变换定义为：

$$\hat{f}(\xi) = \int_{\mathbb{R}^n} f(x) e^{-2\pi i x \cdot \xi} dx$$

**定理 1.1** (Hausdorff-Young不等式)
设 $1 \leqqq p \leqqq 2$，$\frac{1}{p} + \frac{1}{p'} = 1$，则：

$$\|\hat{f}\|_{L^{p'}} \leqqq \|f\|_{L^p}$$
...
$$
\psi(x) = \begin{cases}
1, & 0 \leqqq x < \frac{1}{2} \\
-1, & \frac{1}{2} \leqqq x < 1 \\
0, & \text{其他}
\end{cases}
$$
...
1. $|K(x)| \leqqq \frac{C}{|x|^n}$
2. $|\nabla K(x)| \leqqq \frac{C}{|x|^{n+1}}$
3. $\int_{|x| > 2|y|} |K(x-y) - K(x)| dx \leqqq C$
