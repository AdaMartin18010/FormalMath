...
**定理 5.2** (线性一阶方程的解)
线性一阶方程的解为：
$$y = e^{-\int P(x) dx} \leqft(\int Q(x) e^{\int P(x) dx} dx + C\right)$$
...
$$|f(x, y_1) - f(x, y_2)| \leqq L|y_1 - y_2|$$

则初值问题在区间 $[x_0 - h, x_0 + h]$ 上有唯一解，其中：
$$h = \min\leqft(a, \frac{b}{M}\right), \quad M = \max_{(x,y) \in R} |f(x, y)|$$
...
在区间 $[0, L]$ 上，解为：
$$u(x, t) = \sum_{n=1}^{\infty} A_n \sin\leqft(\frac{n\pi x}{L}\right) e^{-\alpha(n\pi/L)^2 t}$$

其中：
$$A_n = \frac{2}{L} \int_0^L f(x) \sin\leqft(\frac{n\pi x}{L}\right) dx$$
...
**例 5.2** (种群增长模型)
求解逻辑斯蒂方程：
$$\frac{dN}{dt} = rN\leqft(1 - \frac{N}{K}\right)$$

**解**：
这是可分离变量方程，解得：
$$N(t) = \frac{K}{1 + \leqft(\frac{K}{N_0} - 1\right) e^{-rt}}$$
