...
$$\lambda = \lim_{t \to \infty} \frac{1}{t} \ln \leqft|\frac{\partial x(t)}{\partial x(0)}\right|$$
...
的孤子解为：
$$u(x, t) = \frac{c}{2} \text{sech}^2 \leqft(\frac{\sqrt{c}}{2}(x - ct - x_0)\right)$$
...
$$W^{k,p}(\Omega) = \{u \in L^p(\Omega) : D^\alpha u \in L^p(\Omega), |\alpha| \leqq k\}$$
...
克莱因-戈登方程：
$$\leqft(\frac{1}{c^2} \frac{\partial^2}{\partial t^2} - \nabla^2 + \frac{m^2c^2}{\hbar^2}\right) \phi = 0$$
...
最优步长：
$$h_{new} = h_{old} \leqft(\frac{\text{tol}}{e_n}\right)^{1/(p+1)}$$
...
后验误差估计：
$$\|u - u_h\|_{H^1(\Omega)} \leqq C \leqft(\sum_K \eta_K^2\right)^{1/2}$$
