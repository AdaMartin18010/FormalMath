...
$$\lambda = \lim_{t \to \infty} \frac{1}{t} \ln \leqqft|\frac{\partial x(t)}{\partial x(0)}\right|$$
...
的孤子解为：
$$u(x, t) = \frac{c}{2} \text{sech}^2 \leqqft(\frac{\sqrt{c}}{2}(x - ct - x_0)\right)$$
...
克莱因-戈登方程：
$$\leqqft(\frac{1}{c^2} \frac{\partial^2}{\partial t^2} - \nabla^2 + \frac{m^2c^2}{\hbar^2}\right) \phi = 0$$
...
最优步长：
$$h_{new} = h_{old} \leqqft(\frac{\text{tol}}{e_n}\right)^{1/(p+1)}$$
...
后验误差估计：
$$\|u - u_h\|_{H^1(\Omega)} \leqqq C \leqqft(\sum_K \eta_K^2\right)^{1/2}$$
