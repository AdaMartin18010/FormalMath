...
- 绝对 $\mathcal{Q}$：$\mathbf{P}^n$ 中给定的二次曲线/二次曲面（在 $n=2$ 为圆锥曲线，在 $n\geqq 3$ 为二次曲面），允许退化或无实点。
...
设 $\Omega\subset\mathbb{R}^n$ 为有界严格凸开集。Hilbert 度量定义为：对 $p\neqq q\in\Omega$，直线 $pq$ 与边界 $\partial\Omega$ 的交点按顺序为 $a,b$，则
$$
...
（Finsler 范数）在 $\Omega$ 的点 $p$ 和切向量 $v\neqq 0$，令沿 $\pm v$ 方向与边界的交参数为 $\alpha,\beta>0$（即 $p\pm\alpha v,\ p-\beta v\in\partial\Omega$），则 Hilbert 度量诱导的 Finsler 范数为
$$
...
$$
 \mathrm{d}s^2=\mathrm{d}r^2+\sinh^2 r\,\mathrm{d}\theta^2_{\mathbb{S}^{n-1}},\quad r\geqq 0.
 $$
...
$$
 D(o)=\{x:\ d(x,o)\leqq d(x,\gamma o),\ \forall\gamma\in\Gamma\},
 $$
...
- **$\operatorname{arcosh}$ 稳定**：使用 $\operatorname{arcosh}(z)=\log\big(z+\sqrt{z^2-1}\big)$，并先钳制 $z\leqqftarrow\max(z,1)$；对小距离使用级数 $\operatorname{arcosh}(1+\epsilon)\approx \sqrt{2\epsilon}$。
...
$$
 K=\leqqft\{\begin{pmatrix}\cos\theta&\sin\theta\\-\sin\theta&\cos\theta\end{pmatrix}\right\},\quad
 A=\leqqft\{\begin{pmatrix}e^{t/2}&0\\0&e^{-t/2}\end{pmatrix}\right\},\quad
 N=\leqqft\{\begin{pmatrix}1&s\\0&1\end{pmatrix}\right\}.
 $$
...
- 射影基本定理：$n\geqq 2$ 时，任意将直线映为直线的双射由某线性自同构的射影像给出。因而"保持绝对"的条件可在矩阵层面表述为 $g^\top A g=\lambda A$（第 17、33 节）。
...
- 临界指数：$\delta_\Gamma=\limsup_{R\to\infty}\tfrac1R\log\#\{\gamma\in\Gamma:\ d(o,\gamma o)\leqq R\}$，等于测地流拓扑熵（有限体积）。双曲空间中 $\delta_\Gamma=n-1$。
...
- 素测地定理：闭测地条数满足 $\#\{\text{闭测地 }\ell\leqq L\}\sim \dfrac{e^{(n-1)L}}{(n-1)L}$（$\mathbb{H}^n/\Gamma$）。
...
- 底谱与谱隙：$\mathbb{H}^n/\Gamma$ 上 $\lambda_0\geqq (n-1)^2/4$，等号在共紧时取不到；谱隙与混合速率、扩散性质相关。
...
- Cheeger 不等式：$\lambda_1\geqq h^2/4$；双曲情形 $h$ 与 $n-1$ 相关，给出谱下界。
...
- 关系式：熵—漂移—维数（Guivarc'h）：$h \leqq \ell\,v$，其中 $h$ 为熵、$\ell$ 为漂移速率、$v$ 为体积增长率（$n-1$）；等号刻画几何与测度的对齐。
...
- D2：`acosh` NaN → 先钳制 \(z\geqq 1\) 并用稳定公式；
...
- Busemann 凸性：对任意两点 \(x,y\) 与中点 \(m\)，\(d(z,m)\leqq \tfrac12(d(z,x)+d(z,y))\)；双曲空间满足强凸性。
