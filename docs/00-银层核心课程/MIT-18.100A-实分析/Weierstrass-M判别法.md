...
**习题 1.1**。判断级数 $\sum_{n=1}^{\infty} 
rac{\sin(nx)}{n^2}$ 在 $\mathbb{R}$ 上是否一致收敛。

*解答*：$\left|
rac{\sin(nx)}{n^2}
ight| \leq 
rac{1}{n^2} = M_n$。$\sum M_n$ 收敛（p-级数，p=2>1）。由 Weierstrass M-判别法，原级数在 $\mathbb{R}$ 上一致收敛。$\square$
...
*解答*：取 $f_n(x) = 
rac{(-1)^n}{n}$（常数函数）。$\sum f_n$ 在 $\mathbb{R}$ 上一致收敛（交错级数），但 $\sum |f_n| = \sum 
rac{1}{n}$ 发散。$\square$
