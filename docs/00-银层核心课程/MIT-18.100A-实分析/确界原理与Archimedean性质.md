...
**定理陈述**: 设 $S \subseteq \mathbb{R}$ 为非空子集。若 $S$ 有上界，则 $S$ 必有上确界，即存在唯一的实数 $s = \sup S$ 满足**定义 1.2** 中的两个条件。同理，若 $S$ 有下界，则 $S$ 必有下确界。
...
再考虑**例题 3.2** 中的 $T = \{1 - \frac{1}{n} : n \in \mathbb{N}^+\}$。这里 $\sup T = 1$，但 $1 \notin T$，故最大值不存在。
