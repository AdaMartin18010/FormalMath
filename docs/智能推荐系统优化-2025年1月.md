...
## 离线评估脚本样例 / Offline Evaluation Script Examples

```python
# offline_eval.py
# 依赖: numpy, pandas, scikit-learn (可选), tqdm
# 目的: 对离线日志进行CTR@K、Precision@K、Recall@K、NDCG@K、MAP等评估
...
def precision_at_k(recommended: List[str], ground_truth: List[str], k: int = K) -> float:
    rec_k = recommended[:k]
    if not rec_k:
        return 0.0
    hit = sum(1 for i in rec_k if i in ground_truth)
    return hit / len(rec_k)

def recall_at_k(recommended: List[str], ground_truth: List[str], k: int = K) -> float:
    if not ground_truth:
        return 0.0
    rec_k = recommended[:k]
    hit = sum(1 for i in rec_k if i in ground_truth)
...
def dcg_at_k(recommended: List[str], ground_truth: List[str], k: int = K) -> float:
    rec_k = recommended[:k]
    dcg = 0.0
...
def idcg_at_k(ground_truth: List[str], k: int = K) -> float:
    ideal_hits = min(len(ground_truth), k)
    return sum(1.0 / math.log2(i + 2) for i in range(ideal_hits))

def ndcg_at_k(recommended: List[str], ground_truth: List[str], k: int = K) -> float:
    idcg = idcg_at_k(ground_truth, k)
    if idcg == 0:
        return 0.0
    return dcg_at_k(recommended, ground_truth, k) / idcg

def map_at_k(recommended: List[str], ground_truth: List[str], k: int = K) -> float:
    rec_k = recommended[:k]
    if not rec_k:
        return 0.0
    ap_sum, hit = 0.0, 0
    for i, item in enumerate(rec_k):
        if item in ground_truth:
            hit += 1
            ap_sum += hit / (i + 1)
    return ap_sum / max(1, min(len(ground_truth), k))

def evaluate_session(rec_list: List[str], gt_list: List[str]) -> Dict[str, float]:
    return {
        "precision@10": precision_at_k(rec_list, gt_list, 10),
        "recall@10": recall_at_k(rec_list, gt_list, 10),
        "ndcg@10": ndcg_at_k(rec_list, gt_list, 10),
        "map@10": map_at_k(rec_list, gt_list, 10)
    }

if __name__ == "__main__":
    # 输入: 每行一个json: {"user_id":..., "recommended":[...], "clicked":[...]} 或 {"labels":[...]}
    import sys
    import statistics
    ...
    for line in sys.stdin:
        ...
        gt = record.get("clicked", record.get("labels", []))
        m = evaluate_session(rec, gt)
        for k, v in m.items():
            metrics[k].append(v)
    summary = {k: statistics.mean(v) if v else 0.0 for k, v in metrics.items()}
    print(json.dumps({"metrics": summary}, ensure_ascii=False, indent=2))
```

- 输入数据建议通过日志导出形成JSONL：`user_id, recommended[], clicked[]/labels[]`。
- 指标可扩展：Coverage、Novelty、Serendipity、Diversity（基于内容或图谱相似度）。

## A/B 实验配置样例 / A/B Experiment Configuration Examples

```yaml
# ab_experiment.yaml
experiment_id: "reco-rerank-2025Q1"
variants:
  - id: control
    description: "现网模型 v1 (UserCF + Content-based)"
    traffic: 0.5
  - id: treatment
    description: "引入NeuralRerank v2 + 规则融合"
    traffic: 0.5
assignment:
  salt: "formalmath-ab-2025"
  unit: "user_id"   # or session_id
metrics:
  primary:
    - name: "ctr"
      window: "7d"
    - name: "session_recalls@10"
      window: "7d"
  secondary:
    - name: "avg_time_on_page"
    - name: "bounce_rate"
    - name: "save_or_star_rate"
statistical_tests:
  method: "welch_t_test"   # 或非参检验mann_whitney_u
  alpha: 0.05
...
logging:
  sink: "s3://ab-logs/formalmath/2025Q1/"
  schema: "jsonl"
```

```json
// ab_assignment_example.jsonl (每行一个用户分配)
{"user_id": "u123", "experiment_id": "reco-rerank-2025Q1", "variant": "control", "ts": 1735584000}
{"user_id": "u456", "experiment_id": "reco-rerank-2025Q1", "variant": "treatment", "ts": 1735584010}
```

- 实验要点：
  - 随机分流前置去重/粘滞分配，防止跨变体污染。
  - 设定最小可检测效应（MDE）与样本量估算，避免过早停试。
  - 指标分层：全量 + 关键细分（新用户、不同学科页等）。
  - 统计检验与多重比较校正（必要时用FDR）。
