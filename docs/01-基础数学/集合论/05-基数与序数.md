...
$$A \sim B \leqftrightarrow \exists f : A \rightarrow B, f \text{ 是双射}$$
...
- **等势性**: 两个集合有相同基数当且仅当它们等势
  - 形式化：$|A| = |B| \leqftrightarrow A \sim B$
- **序数性**: 每个基数都是序数
...
- **等势性**: $|A| = |B| \leqftrightarrow A \sim B$
- **序数性**: 每个基数都是序数
- **良序性**: 基数在$\in$关系下构成良序集
...
**定义 5.5** (基数的大小关系)
设 $A$ 和 $B$ 是集合，定义：
$$|A| \leqq |B| \leqftrightarrow \exists f : A \rightarrow B, f \text{ 是单射}$$
$$|A| < |B| \leqftrightarrow |A| \leqq |B| \wedge |A| \neqq |B|$$

**定理 5.2** (康托尔-伯恩斯坦定理 / Cantor-Bernstein Theorem)
如果 $|A| \leqq |B|$ 且 $|B| \leqq |A|$，则 $|A| = |B|$。
...
**证明**：
首先，$|A| \leqq |\mathcal{P}(A)|$，因为函数 $f(x) = \{x\}$ 是单射。
...
偏序集 $(A, \leqq)$ 是良序集，当且仅当 $A$ 的每个非空子集都有最小元素：
$$\forall S \subseteq A, S \neqq \emptyset \rightarrow \exists m \in S, \forall s \in S, m \leqq s$$

**定义 5.11** (序型 / Order Type)
两个良序集 $(A, \leqq_A)$ 和 $(B, \leqq_B)$ 有相同的序型，当且仅当存在保序双射 $f : A \rightarrow B$：
$$f \text{ 是双射} \wedge \forall x, y \in A, x \leqq_A y \rightarrow f(x) \leqq_B f(y)$$
...
- **非良序集**: 不是所有良序集都是序数（如$(\mathbb{Z}, \leqq)$不是序数）
...
**集合论定义**:
序数$\alpha$定义为：
$$\alpha \text{是序数} \leqftrightarrow \alpha \text{是传递集且} (\alpha, \in) \text{是良序集}$$
...
$$\kappa \leqq \lambda \vee \lambda \leqq \kappa$$
