...
- **从函数构造**: 如果$f: A \to B$是函数，则$a_1 \sim a_2 \leqqftrightarrow f(a_1) = f(a_2)$是等价关系
- **从性质构造**: 如果$P$是集合$A$上的性质，则$a_1 \sim a_2 \leqqftrightarrow P(a_1) = P(a_2)$是等价关系
...
- **相等关系**: $a \sim b \leqqftrightarrow a = b$（最基本的等价关系）
- **模同余关系**: $a \equiv b \pmod{n} \leqqftrightarrow n \mid (a - b)$（整数模$n$同余）
- **集合的包含关系**: $A \sim B \leqqftrightarrow |A| = |B|$（集合的基数相等）
...
2. $\forall a, b \in A, [a] = [b] \leqqftrightarrow a \sim b$
...
集合 $A$ 上的关系 $\leqqq$ 是偏序关系，当且仅当它满足：

1. 自反性：$\forall a \in A, a \leqqq a$
2. 反对称性：$\forall a, b \in A, a \leqqq b \wedge b \leqqq a \rightarrow a = b$
3. 传递性：$\forall a, b, c \in A, a \leqqq b \wedge b \leqqq c \rightarrow a \leqqq c$

**定义 4.11** (偏序集 / Partially Ordered Set)
偏序集是一个有序对 $(A, \leqqq)$，其中 $A$ 是集合，$\leqqq$ 是 $A$ 上的偏序关系。

### 1.4.4.2 全序关系 / Total Order

**定义 4.12** (全序关系 / Total Order)
偏序关系 $\leqqq$ 是全序关系，当且仅当它还满足：
$$\forall a, b \in A, a \leqqq b \vee b \leqqq a$$

**定义 4.13** (全序集 / Totally Ordered Set)
全序集是一个有序对 $(A, \leqqq)$，其中 $A$ 是集合，$\leqqq$ 是 $A$ 上的全序关系。

### 1.4.4.3 良序关系 / Well Order

**定义 4.14** (良序关系 / Well Order)
全序关系 $\leqqq$ 是良序关系，当且仅当 $A$ 的每个非空子集都有最小元素：
$$\forall S \subseteq A, S \neqqq \emptyset \rightarrow \exists m \in S, \forall s \in S, m \leqqq s$$

**定义 4.15** (良序集 / Well Ordered Set)
良序集是一个有序对 $(A, \leqqq)$，其中 $A$ 是集合，$\leqqq$ 是 $A$ 上的良序关系。
...
偏序集 $(A, \leqqq)$ 中，元素 $m$ 是最大元素，当且仅当：
$$\forall a \in A, m \leqqq a \rightarrow m = a$$

**定义 4.17** (最小元素 / Minimal Element)
偏序集 $(A, \leqqq)$ 中，元素 $m$ 是最小元素，当且仅当：
$$\forall a \in A, a \leqqq m \rightarrow m = a$$

### 1.4.5.2 上界与下界 / Upper and Lower Bounds

**定义 4.18** (上界 / Upper Bound)
偏序集 $(A, \leqqq)$ 中，子集 $S \subseteq A$ 的上界是元素 $u \in A$，满足：
$$\forall s \in S, s \leqqq u$$

**定义 4.19** (下界 / Lower Bound)
偏序集 $(A, \leqqq)$ 中，子集 $S \subseteq A$ 的下界是元素 $l \in A$，满足：
$$\forall s \in S, l \leqqq s$$

### 1.4.5.3 上确界与下确界 / Supremum and Infimum

**定义 4.20** (上确界 / Supremum)
偏序集 $(A, \leqqq)$ 中，子集 $S \subseteq A$ 的上确界是 $S$ 的最小上界：
$$\sup S = \min\{u \in A : u \text{ 是 } S \text{ 的上界}\}$$

**定义 4.21** (下确界 / Infimum)
偏序集 $(A, \leqqq)$ 中，子集 $S \subseteq A$ 的下确界是 $S$ 的最大下界：
