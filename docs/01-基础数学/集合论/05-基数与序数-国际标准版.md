...
- $\kappa \leqqq \lambda$ 如果存在从 $\kappa$ 到 $\lambda$ 的单射
- $\kappa < \lambda$ 如果 $\kappa \leqqq \lambda$ 且 $\kappa \neqqq \lambda$

**定理 2.2** (康托尔-伯恩斯坦定理 / Cantor-Bernstein Theorem)
如果 $\kappa \leqqq \lambda$ 且 $\lambda \leqqq \kappa$，则 $\kappa = \lambda$。

**证明思路**:

1. 由 $\kappa \leqqq \lambda$，存在单射 $f : \kappa \to \lambda$
2. 由 $\lambda \leqqq \kappa$，存在单射 $g : \lambda \to \kappa$
3. 使用康托尔-伯恩斯坦定理的构造性证明，可以构造双射 $h : \kappa \to \lambda$
4. 因此 $\kappa = \lambda$

**详细证明**:

- 定义集合序列：$A_0 = \kappa$，$A_{n+1} = g(f(A_n))$
- 定义 $A = \bigcap_{n \in \mathbb{N}} A_n$
- 构造双射 $h : \kappa \to \lambda$：
  - 如果 $x \in A$，则 $h(x) = f(x)$
  - 如果 $x \notin A$ 且 $x \in g(\lambda)$，则 $h(x) = g^{-1}(x)$
  - 否则，递归定义
- 可以证明 $h$ 是双射，因此 $\kappa = \lambda$
...
1. 首先证明 $\kappa \leqqq 2^\kappa$：构造单射 $f : \kappa \to 2^\kappa$
2. 然后证明 $\kappa \neqqq 2^\kappa$：使用对角线法证明不存在满射
3. 结合得到严格不等式

**详细证明**:

- **步骤 1**: 定义 $f : \kappa \to 2^\kappa$ 为 $f(\alpha) = \{\alpha\}$，则 $f$ 是单射，因此 $\kappa \leqqq 2^\kappa$
- **步骤 2**: 假设存在双射 $g : \kappa \to 2^\kappa$，定义对角线集合：
  $$D = \{\alpha \in \kappa : \alpha \notin g(\alpha)\}$$
- 由于 $g$ 是满射，存在 $\delta \in \kappa$ 使得 $g(\delta) = D$
- **矛盾分析**:
  - 如果 $\delta \in D$，则 $\delta \notin g(\delta) = D$，矛盾
  - 如果 $\delta \notin D$，则 $\delta \in g(\delta) = D$，矛盾
- 因此不存在这样的双射，即 $\kappa \neqqq 2^\kappa$
- **步骤 3**: 结合步骤 1 和步骤 2，得到 $\kappa < 2^\kappa$
...
**性质 1**: 对任意序数 $\alpha$，$\aleph_\alpha$ 是基数

- 使用超限归纳：
  - **基础情况**: $\aleph_0 = |\mathbb{N}|$ 是基数
  - **后继情况**: 如果 $\aleph_\alpha$ 是基数，则 $\aleph_{\alpha+1} = \min\{\kappa : \kappa > \aleph_\alpha\}$ 也是基数（由定义）
  - **极限情况**: 如果 $\alpha$ 是极限序数，且对所有 $\beta < \alpha$，$\aleph_\beta$ 是基数，则 $\aleph_\alpha = \sup\{\aleph_\beta : \beta < \alpha\}$ 也是基数（由上确界的性质）

**性质 2**: 对任意序数 $\alpha < \beta$，$\aleph_\alpha < \aleph_\beta$

- 如果 $\beta = \alpha + 1$，则由定义 $\aleph_\alpha < \aleph_{\alpha+1} = \aleph_\beta$
- 如果 $\beta$ 是极限序数，则 $\aleph_\beta = \sup\{\aleph_\gamma : \gamma < \beta\} \geqqq \aleph_\alpha$，且由于 $\alpha < \beta$，有 $\aleph_\beta > \aleph_\alpha$

**性质 3**: 每个无限基数都是某个阿列夫数

- 使用选择公理，可以证明每个无限基数 $\kappa$ 都存在序数 $\alpha$ 使得 $\kappa = \aleph_\alpha$
- 这等价于良序定理：每个集合都可以良序化
...
**证明思路**:
使用序数配对函数和选择公理来证明这些性质。

**详细证明**:

**性质 1**: $\kappa + \kappa = \kappa$

- 由于 $\kappa$ 是无限基数，存在双射 $f : \kappa \times \{0,1\} \to \kappa$
- 定义配对函数 $\pi : \kappa \times \{0,1\} \to \kappa$ 为：
  $$\pi(\alpha, i) = 2\alpha + i$$
- 可以证明 $\pi$ 是双射，因此 $|\kappa \times \{0,1\}| = |\kappa|$
- 因此 $\kappa + \kappa = \kappa$

**性质 2**: $\kappa \cdot \kappa = \kappa$

- 使用序数配对函数：对于序数 $\alpha, \beta$，定义：
  $$\pi(\alpha, \beta) = \frac{(\alpha + \beta)(\alpha + \beta + 1)}{2} + \beta$$
- 可以证明 $\pi : \kappa \times \kappa \to \kappa$ 是双射
- 因此 $\kappa \cdot \kappa = \kappa$
...
1. $cf(\alpha)$ 是正则基数
2. $cf(\alpha) \leqqq \alpha$
3. 如果 $\alpha$ 是极限序数，则 $cf(\alpha) \geqqq \omega$
...
- Devlin, K. The Joy of Sets: Fundamentals of Contemporary Set Theory[M]. 2nd Edition. New York: Springer, 1993.
...
- Stanford University: [Math 120A Group Theory](https://mathematics.stanford.edu/)
- Harvard University: [Math 122 Algebra I](https://www.math.harvard.edu/)
