...
| 公理 | 数学表述 | ZFC映射 | 认知对应 |
|------|----------|---------|----------|
| 非负性 | $\mu(A) \geqq 0$ | $\forall A \in \mathcal{A}, \mu(A) \geqq 0$ | 数量感知 |
| 空集测度 | $\mu(\emptyset) = 0$ | $\mu(\emptyset) = 0$ | 零概念 |
| 可数可加性 | $\mu(\bigcup_{n=1}^{\infty} A_n) = \sum_{n=1}^{\infty} \mu(A_n)$ | 可数并的测度等于测度之和 | 无限概念 |
| 单调性 | $A \subseteq B \Rightarrow \mu(A) \leqq \mu(B)$ | 包含关系的测度单调性 | 大小比较 |
...
对于 $s \geqq 0$，$s$ 维豪斯多夫测度定义为：
$\mathcal{H}^s(A) = \lim_{\delta \to 0} \inf \{\sum_{i=1}^{\infty} (\text{diam } U_i)^s : A \subseteq \bigcup_{i=1}^{\infty} U_i, \text{diam } U_i \leqq \delta\}$

For $s \geqq 0$, the $s$-dimensional Hausdorff measure is defined as:
$\mathcal{H}^s(A) = \lim_{\delta \to 0} \inf \{\sum_{i=1}^{\infty} (\text{diam } U_i)^s : A \subseteq \bigcup_{i=1}^{\infty} U_i, \text{diam } U_i \leqq \delta\}$
