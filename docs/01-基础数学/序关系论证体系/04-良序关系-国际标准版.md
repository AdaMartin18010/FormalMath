...
良序集是一个有序对 $(A, \leqq)$，其中 $A$ 是一个集合，$\leqq$ 是 $A$ 上的良序关系。

A well-ordered set is an ordered pair $(A, \leqq)$, where $A$ is a set and $\leqq$ is a well-ordered relation on $A$.
...
在良序集 $(A, \leqq)$ 中，不存在无限下降链。

In a well-ordered set $(A, \leqq)$, there is no infinite descending chain.
...
在良序集 $(A, \leqq)$ 中，如果性质 $P$ 满足：
$\forall a \in A(\forall b < a(P(b)) \rightarrow P(a))$
则 $\forall a \in A(P(a))$。

In a well-ordered set $(A, \leqq)$, if property $P$ satisfies:
$\forall a \in A(\forall b < a(P(b)) \rightarrow P(a))$
...
设 $(A, \leqq)$ 是良序集，对于 $a \in A$，初始段 $S_a$ 定义为：
$S_a = \{x \in A : x < a\}$

Let $(A, \leqq)$ be a well-ordered set, for $a \in A$, the initial segment $S_a$ is defined as:
$S_a = \{x \in A : x < a\}$
...
设 $(A, \leqq)$ 是良序集，对于 $a \in A$，如果存在 $b \in A$ 使得 $a < b$ 且不存在 $c \in A$ 满足 $a < c < b$，则称 $b$ 是 $a$ 的后继元。

Let $(A, \leqq)$ be a well-ordered set, for $a \in A$, if there exists $b \in A$ such that $a < b$ and there is no $c \in A$ satisfying $a < c < b$, then $b$ is called the successor of $a$.
...
$(\mathbb{N}, \leqq)$ is a well-ordered set.
...
1. **自反性** / **Reflexivity**: $\forall a \in A(a \leqq a)$
2. **反对称性** / **Antisymmetry**: $\forall a, b \in A(a \leqq b \land b \leqq a \rightarrow a = b)$
3. **传递性** / **Transitivity**: $\forall a, b, c \in A(a \leqq b \land b \leqq c \rightarrow a \leqq c)$
4. **完全性** / **Totality**: $\forall a, b \in A(a \leqq b \lor b \leqq a)$
