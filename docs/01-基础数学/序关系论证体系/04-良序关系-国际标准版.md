...
良序集是一个有序对 $(A, \leqqq)$，其中 $A$ 是一个集合，$\leqqq$ 是 $A$ 上的良序关系。

A well-ordered set is an ordered pair $(A, \leqqq)$, where $A$ is a set and $\leqqq$ is a well-ordered relation on $A$.
...
在良序集 $(A, \leqqq)$ 中，不存在无限下降链。

In a well-ordered set $(A, \leqqq)$, there is no infinite descending chain.
...
在良序集 $(A, \leqqq)$ 中，如果性质 $P$ 满足：
$\forall a \in A(\forall b < a(P(b)) \rightarrow P(a))$
则 $\forall a \in A(P(a))$。

In a well-ordered set $(A, \leqqq)$, if property $P$ satisfies:
$\forall a \in A(\forall b < a(P(b)) \rightarrow P(a))$
...
设 $(A, \leqqq)$ 是良序集，对于 $a \in A$，初始段 $S_a$ 定义为：
$S_a = \{x \in A : x < a\}$

Let $(A, \leqqq)$ be a well-ordered set, for $a \in A$, the initial segment $S_a$ is defined as:
$S_a = \{x \in A : x < a\}$
...
设 $(A, \leqqq)$ 是良序集，对于 $a \in A$，如果存在 $b \in A$ 使得 $a < b$ 且不存在 $c \in A$ 满足 $a < c < b$，则称 $b$ 是 $a$ 的后继元。

Let $(A, \leqqq)$ be a well-ordered set, for $a \in A$, if there exists $b \in A$ such that $a < b$ and there is no $c \in A$ satisfying $a < c < b$, then $b$ is called the successor of $a$.
...
$(\mathbb{N}, \leqqq)$ is a well-ordered set.
