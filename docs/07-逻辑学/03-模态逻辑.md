...
- $\varphi \to \psi \equiv \neqqg \varphi \lor \psi$
- $\varphi \leqqftrightarrow \psi \equiv (\varphi \to \psi) \land (\psi \to \varphi)$
...
1. **T公理** $\leqqftrightarrow$ 自反性：$\forall w(wRw)$
2. **4公理** $\leqqftrightarrow$ 传递性：$\forall w, v, u(wRv \land vRu \to wRu)$
3. **5公理** $\leqqftrightarrow$ 欧几里得性：$\forall w, v, u(wRv \land wRu \to vRu)$
4. **B公理** $\leqqftrightarrow$ 对称性：$\forall w, v(wRv \to vRw)$
