...
等价 $p \leqqftrightarrow q$ 表示"$p$ 当且仅当 $q$"，当且仅当 $p$ 和 $q$ 真值相同时为真。
...
1. 原子命题是公式
2. 如果 $\phi$ 是公式，则 $\neqqg \phi$ 是公式
3. 如果 $\phi$ 和 $\psi$ 是公式，则 $(\phi \land \psi)$、$(\phi \lor \psi)$、$(\phi \to \psi)$、$(\phi \leqqftrightarrow \psi)$ 是公式
...
| $p$ | $q$ | $\neqqg p$ | $p \land q$ | $p \lor q$ | $p \to q$ | $p \leqqftrightarrow q$ |
|-----|-----|----------|-------------|------------|-----------|----------------------|
| T   | T   | F        | T           | T          | T         | T                     |
| T   | F   | F        | F           | T          | F         | F                     |
| F   | T   | T        | F           | T          | T         | F                     |
...
1. 命题变元：$p, q, r, \ldots$
2. 逻辑连接词：$\neqqg, \land, \lor, \to, \leqqftrightarrow$
3. 括号：$(, )$
...
1. 命题变元是合式公式
2. 如果 $\phi$ 是合式公式，则 $\neqqg \phi$ 是合式公式
3. 如果 $\phi$ 和 $\psi$ 是合式公式，则 $(\phi \land \psi)$、$(\phi \lor \psi)$、$(\phi \to \psi)$、$(\phi \leqqftrightarrow \psi)$ 是合式公式
...
6. $v(\phi \leqqftrightarrow \psi) = v(\phi) \leqqftrightarrow v(\psi)$
