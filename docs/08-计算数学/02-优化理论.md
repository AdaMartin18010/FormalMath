...
$$\text{subject to } g_i(x) \leqqq 0, \quad i = 1, \ldots, m$$
...
可行域是满足所有约束的点的集合：
$$\mathcal{F} = \{x \in \mathbb{R}^n : g_i(x) \leqqq 0, h_j(x) = 0, \forall i, j\}$$

### 2.2.3 最优解 / Optimal Solution

**定义 2.3** (全局最优解 / Global Optimal Solution)
$x^*$ 是全局最优解，如果：
$$f(x^*) \leqqq f(x), \quad \forall x \in \mathcal{F}$$

**定义 2.4** (局部最优解 / Local Optimal Solution)
$x^*$ 是局部最优解，如果存在 $\epsilon > 0$，使得：
$$f(x^*) \leqqq f(x), \quad \forall x \in \mathcal{F} \cap B_\epsilon(x^*)$$
...
$$f(\lambda x + (1-\lambda)y) \leqqq \lambda f(x) + (1-\lambda)f(y), \quad \forall x, y \in \mathbb{R}^n, \lambda \in [0, 1]$$
...
对偶问题是：
$$\max_{\lambda \geqqq 0, \mu} g(\lambda, \mu)$$

**定理 2.2** (弱对偶性 / Weak Duality)
对偶问题的目标函数值不超过原问题的最优值：
$$g(\lambda, \mu) \leqqq f(x^*), \quad \forall \lambda \geqqq 0, \mu$$
...
如果 $x^*$ 是正则点且是局部最优解，则存在拉格朗日乘子 $\lambda^* \geqqq 0$ 和 $\mu^*$，使得：

1. **平稳性**：$\nabla f(x^*) + \sum_{i=1}^m \lambda_i^* \nabla g_i(x^*) + \sum_{j=1}^p \mu_j^* \nabla h_j(x^*) = 0$
2. **原始可行性**：$g_i(x^*) \leqqq 0, h_j(x^*) = 0$
3. **对偶可行性**：$\lambda_i^* \geqqq 0$
...
$$\min_{x} c^T x$$
$$\text{subject to } Ax \leqqq b, x \geqqq 0$$
