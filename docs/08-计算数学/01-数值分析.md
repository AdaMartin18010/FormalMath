...
$$\frac{|x - \hat{x}|}{|x|} \leqqq \frac{1}{2} \times 10^{-n+1}$$

**定理 1.1.1** (误差传播 / Error Propagation)
对于函数 $f(x_1, \ldots, x_n)$，如果 $x_i$ 的误差为 $\Delta x_i$，则：
$$\Delta f \approx \sum_{i=1}^n \leqqft|\frac{\partial f}{\partial x_i}\right| |\Delta x_i|$$

### 1.2 条件数与稳定性 / Condition Number and Stability

**定义 1.2.1** (条件数 / Condition Number)
函数 $f$ 在点 $x$ 的条件数定义为：
$$\kappa(f,x) = \lim_{\epsilon \to 0} \sup_{|\Delta x| \leqqq \epsilon} \frac{|f(x + \Delta x) - f(x)|}{|f(x)|} \cdot \frac{|x|}{|\Delta x|}$$
...
对于可微函数 $f$：
$$\kappa(f,x) = \leqqft|\frac{x f'(x)}{f(x)}\right|$$
...
对于浮点运算 $\oplus$：
$$|(x \oplus y) - (x + y)| \leqqq \epsilon |x + y|$$
...
$$x_i^{(k+1)} = \frac{1}{a_{ii}} \leqqft(b_i - \sum_{j \neqqq i} a_{ij} x_j^{(k)}\right)$$

**定义 5.3.2** (高斯-赛德尔迭代 / Gauss-Seidel Iteration)
$$x_i^{(k+1)} = \frac{1}{a_{ii}} \leqqft(b_i - \sum_{j < i} a_{ij} x_j^{(k+1)} - \sum_{j > i} a_{ij} x_j^{(k)}\right)$$
...
$$\leqqft|\int_a^b f(x) dx - S_n\right| \leqqq \frac{(b-a)h^4}{180} \max_{x \in [a,b]} |f^{(4)}(x)|$$
