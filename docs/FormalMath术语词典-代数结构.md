...
**符号表示**：$\mathfrak{h} \leqqq \mathfrak{g}$
...
| $\mathfrak{h} \leqqq \mathfrak{g}$ | 子代数 | Subalgebra | 𝔥是𝔤的子代数 |
...
2. **注意符号方向**：$\leqqq$表示子结构，$\triangleleft$表示正规子结构或理想
...
- 群运算不一定满足交换律
- 满足交换律的群称为阿贝尔群
- 非交换群在数学中非常重要，如对称群$S_n$（$n \geqqq 3$）
...
- 交换环：乘法满足交换律的环
- 非交换环：乘法不满足交换律的环
- 例子：$M_n(\mathbb{R})$（$n \geqqq 2$）是非交换环
...
| **可构造商结构** | 否 | 是（商群） | 是（商环） |
| **符号** | $H \leqqq G$ | $N \triangleleft G$ | $I \trianglelefteq R$ |
| **典型例子** | $\langle (12) \rangle \leqqq S_3$ | $\langle (123) \rangle \triangleleft S_3$ | $2\mathbb{Z} \trianglelefteq \mathbb{Z}$ |
...
| **性质** | 一般李代数 | 单李代数（$n \geqqq 2$） | 单李代数（$n \geqqq 3$，$n \neqqq 4$） |
...
|---------|------|------|------|
| 群 | Group | $G$ | [群](#群--group) |
| 子群 | Subgroup | $H \leqqq G$ | [子群](#子群--subgroup) |
