...
$$\mathcal{T} = \leqqft\{\prod_{i \in I} U_i : U_i \in \mathcal{T}_i, \text{ 且只有有限个 } U_i \neqqq X_i\right\}$$
...
- **步骤 4**: 证明连续性
  ...
  - 对于任意 $y \in V$，有 $r \leqqq f(y) \leqqq s$，因此 $|f(y) - f(x)| < \varepsilon$
...
1. **正定性** / Positive definiteness: $d(x,y) \geqqq 0$，且 $d(x,y) = 0$ 当且仅当 $x = y$
2. **对称性** / Symmetry: $d(x,y) = d(y,x)$
3. **三角不等式** / Triangle inequality: $d(x,z) \leqqq d(x,y) + d(y,z)$
...
度量空间 $(X,d)$ 中的序列 $\{x_n\}$ 称为柯西序列，如果对于任意 $\varepsilon > 0$，存在 $N \in \mathbb{N}$ 使得对于任意 $m,n \geqqq N$，$d(x_m,x_n) < \varepsilon$。
