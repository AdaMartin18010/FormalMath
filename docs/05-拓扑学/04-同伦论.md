...
对于 $n \geqqq 1$，$n$ 阶同伦群定义为：
$$\pi_n(X,x_0) = \{[f] : f: (S^n, *) \to (X, x_0)\}$$

**群运算** / Group Operation:

- 对于 $n = 1$：道路复合
- 对于 $n \geqqq 2$：点积 $[\alpha] \cdot [\beta] = [\alpha \cdot \beta]$

**定理 5.4.2.1** (同伦群的基本性质 / Basic Properties of Homotopy Groups)
同伦群具有以下性质：

1. $\pi_n(X,x_0)$ 是阿贝尔群（$n \geqqq 2$）
...
$$\pi_1(S^1) \cong \mathbb{Z}, \quad \pi_n(S^1) = 0 \text{ for } n \geqqq 2$$
...
谱序列收敛到 $H$，如果存在 $r_0$ 使得 $E_r = E_\infty$ 对所有 $r \geqqq r_0$，且 $E_\infty$ 与 $H$ 的某个过滤相关。
...
1. 满射，如果 $k \leqqq 2n-2$
2. 同构，如果 $k \leqqq 2n-3$
