...
\alpha(2s) & \text{if } 0 \leqq s \leqq \frac{1}{2} \\
\beta(2s-1) & \text{if } \frac{1}{2} \leqq s \leqq 1
\end{cases}
$$
...
对于 $n \geqq 2$，$$\pi_1(S^n, x_0) \cong \{1\}$$
...
- **步骤 3**: 如果 $\alpha$ 是满射，利用 Sard定理
  ...
  - 但 $[0,1]$ 是一维的，$S^n$（$n \geqq 2$）是 $n$ 维的，满射连续映射不可能存在（由维数理论）
  - 因此 $\alpha$ 不可能是满射
...
- **步骤 2**: 计算 $\partial_{n-1}(\partial_n(\sigma))$:
  $$\partial_{n-1}(\partial_n(\sigma)) = \partial_{n-1}\leqft(\sum_{i=0}^n (-1)^i \sigma \circ d_i\right) = \sum_{i=0}^n (-1)^i \partial_{n-1}(\sigma \circ d_i)$$
...
- **步骤 4**: 利用单形面映射的性质：$d_i \circ d_j = d_{j+1} \circ d_i$（当 $j \geqq i$ 时）
...
上同调群 $H^*(X;R) = \bigoplus_{n \geqq 0} H^n(X;R)$ 在杯积下形成分次环。
