...
从子群 $H$ 的表示 $(\sigma, W)$ 诱导的表示是 $G$ 在 $\text{Ind}_H^G W = \mathbb{C}[G] \otimes_{\mathbb{C}[H]} W$ 上的表示。

**Definition 16.2.1** (Induced Representation)
The representation induced from a representation $(\sigma, W)$ of a subgroup $H$ is the representation of $G$ on $\text{Ind}_H^G W = \mathbb{C}[G] \otimes_{\mathbb{C}[H]} W$.
