...
- 关系：对于关系符号 $R$，$R^{\mathcal{A}}([a_S]_F) \leqftrightarrow \{S : R^{M_S}(a_S)\} \in F$
...
**定理 1.2** (勒文海姆-斯科伦定理)
如果一阶理论 $T$ 有无限模型，那么对于任意基数 $\kappa \geqq |L| + \aleph_0$，$T$ 都有基数为 $\kappa$ 的模型。
...
- $|cl(X)| \leqq \kappa$（因为 $|L| \leqq \kappa$）
- $cl(X) \models T$（因为 $T$ 是通用理论）

**向上勒文海姆-斯科伦定理的证明**：

**步骤1**：使用紧致性定理
设 $\kappa > |A|$，构造新理论 $T' = T \cup \{c_\alpha \neqq c_\beta : \alpha < \beta < \kappa\}$，其中 $c_\alpha$ 是新常量。
...
2. 选择超滤子 $U$ 在 $I$ 上
3. 定义等价关系：$f \sim g \leqftrightarrow \{i : f(i) = g(i)\} \in U$
