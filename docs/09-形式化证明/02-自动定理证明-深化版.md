...
给定两个子句 $C_1 = A \lor L$ 和 $C_2 = B \lor \neqg L$，其中 $L$ 是字面量，$A$ 和 $B$ 是子句的其余部分，则归结原理定义为：
