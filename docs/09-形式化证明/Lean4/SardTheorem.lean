...
open Manifold MeasureTheory Topology Filter Classical
...
variable {M N : Type u} [TopologicalSpace M] [TopologicalSpace N]
  ...
  [FiniteDimensional ℝ (EuclideanSpace ℝ (Fin m))]
  [FiniteDimensional ℝ (EuclideanSpace ℝ (Fin n))]

-- 临界点集
def CriticalPoints (f : C^∞⟮M, N⟯) : Set M :=
  {p : M | rank (mfderiv (𝓡 m) (𝓡 n) f p) < n}

-- 临界值集
def CriticalValues (f : C^∞⟮M, N⟯) : Set N :=
  f '' (CriticalPoints f)

-- 正则值集
def RegularValues (f : C^∞⟮M, N⟯) : Set N :=
  (CriticalValues f)ᶜ

-- 流形上的测度零集（使用图册）
def MeasureZero (S : Set N) : Prop :=
  ∀ (chart : PartialHomeomorph N (EuclideanSpace ℝ (Fin n))),
    MeasureTheory.volume (chart.toFun '' (S ∩ chart.source)) = 0
...
-- Sard定理的欧氏空间版本（关键步骤）
theorem sard_euclidean {m n : ℕ} (f : (EuclideanSpace ℝ (Fin m)) → 
    (EuclideanSpace ℝ (Fin n))) (hf : ContDiff ℝ ⊤ f) :
