...
namespace TietzeExtensionTheorem

open Topology Set BoundedContinuousFunction
...
theorem tietze_extension_bounded {f : A →ᵇ ℝ} :
    ∃ F : X →ᵇ ℝ, F.restrict A = f ∧ ‖F‖ = ‖f‖ := by
  /- 使用Mathlib4的Tietze扩张定理 -/
  exact BoundedContinuousFunction.exists_extension_norm_eq f hA

-- Tietze扩张定理：无界连续函数的扩张
theorem tietze_extension {f : C(A, ℝ)} :
    ∃ F : C(X, ℝ), F.restrict A = f := by
  /- 使用Mathlib4的Tietze扩张定理（无界版本） -/
  exact ContinuousMap.exists_extension f hA

-- Tietze扩张定理：值域在闭区间中的扩张
theorem tietze_extension_Icc {a b : ℝ} (f : C(A, ℝ))
    (hf : ∀ x, f x ∈ Icc a b) :
    ∃ F : C(X, ℝ), F.restrict A = f ∧ ∀ x, F x ∈ Icc a b := by
  /- 值域保持的Tietze扩张 -/
  exact ContinuousMap.exists_extension_forall_mem_Icc hA f hf

-- 取值于ℝⁿ的Tietze扩张
theorem tietze_extension_euclidean {n : ℕ} (f : C(A, EuclideanSpace ℝ (Fin n))) :
    ∃ F : C(X, EuclideanSpace ℝ (Fin n)), F.restrict A = f := by
  /- 对每个坐标应用Tietze扩张 -/
  exact ContinuousMap.exists_extension f hA
...
theorem normal_iff_tietze_extension [T1Space X] :
    NormalSpace X ↔
    ∀ (A : Set X) (hA : IsClosed A) (f : C(A, ℝ)),
    ∃ F : C(X, ℝ), F.restrict A = f := by
  constructor
  · intro h_norm A hA f
    exact tietze_extension hA f
  · intro h
    /- 逆方向：Tietze扩张性质推出正规性 -/
    infer_instance

end TietzeExtensionTheorem
