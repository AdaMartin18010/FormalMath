...
theorem compact_iff_sequentially_compact {X : Type*} [MetricSpace X] [ProperSpace X]
  ...
  · intro h_compact
    ...
    · intro n
      exact hx n
  · intro h_seq_compact
    /- 序列紧致 ⟹ 紧致（在Proper度量空间中） -/
    rw [← isSeqCompact_iff_isCompact]
    intro x hx
    exact h_seq_compact x hx
...
theorem extreme_value_heine_borel {n : ℕ} {K : Set (Fin n → ℝ)} (hK : IsCompact K)
    {f : (Fin n → ℝ) → ℝ} (hf : ContinuousOn f K) (hne : K.Nonempty) :
    ∃ (x₀ ∈ K), ∀ (x ∈ K), f x ≤ f x₀ := by
  /- 证明思路：
     ...
     3. 有界闭集包含上确界
  -/
  apply IsCompact.exists_isMaxOn hK hf hne
...
theorem uniform_continuity {n : ℕ} {K : Set (Fin n → ℝ)} (hK : IsCompact K)
  ...
  have h_separated : SeparatedSpace (Fin n → ℝ) := by
    infer_instance
  apply compactSpace_uniformity
  · exact hK
  · exact hf
...
theorem infinite_dim_not_heine_borel {X : Type*} [NormedAddCommGroup X]
  ...
  intro h_compact
  /- 构造标准正交基序列 -/
  have h_not_compact : ¬IsCompact (Metric.closedBall (0 : X) 1) := by
    apply not_compact_isClosed_unit_ball_of_infiniteDimensional
  contradiction
...
example {n : ℕ} : IsCompact {x : Fin n → ℝ | ‖x‖ = 1} := by
  ...
  · /- 闭集：范数函数的逆像 -/
    have h_closed : IsClosed {x : Fin n → ℝ | ‖x‖ = 1} := by
      have : {x : Fin n → ℝ | ‖x‖ = 1} = (fun x => ‖x‖) ⁻¹' {1} := by
        ext x
        simp
      rw [this]
    ...
    exact h_closed
  · /- 有界：包含在单位球中 -/
    rw [Bornology.isBounded_iff_forall_norm_lt]
    intro x hx
    simp at hx
    rw [hx]
    norm_num
...
example {n : ℕ} {K : Set (Fin n → ℝ)} (hK : IsCompact K) (hne : K.Nonempty)
  ...
  have h_max : ∃ (y₀ ∈ K), IsMaxOn (fun x => -f x) K y₀ := by
    apply IsCompact.exists_isMaxOn hK
    · apply ContinuousOn.neg hf
    · exact hne
  rcases h_max with ⟨y₀, hy₀, hmax⟩
  use y₀, hy₀
  intro x hx
  have : -f y₀ ≥ -f x := hmax hx
  linarith
```
