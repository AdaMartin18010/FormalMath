...
theorem compactness_theorem :
    T.IsSatisfiable ↔ ∀ (T0 : L.Theory), T0.Finite → T0 ⊆ T → T0.IsSatisfiable := by
  /- 使用Mathlib4的紧致性定理 -/
  exact isSatisfiable_iff_all_finite_subsets_isSatisfiable T

-- 紧致性定理的等价形式：φ 被 T 语义蕴涵 ⟺ 被某个有限子集语义蕴涵
theorem compactness_theorem_alt {φ : L.Sentence} :
    T ⊨ᵇ φ ↔ ∃ (T0 : L.Theory), T0.Finite ∧ T0 ⊆ T ∧ T0 ⊨ᵇ φ := by
  /- 使用Mathlib4的紧致性定理推论 -/
  exact models_sentence_iff_models_subsentence T φ
