...
-- 威尔逊定理的整数版本
theorem wilson_theorem_nat {p : ℕ} (hp : p.Prime) :
    (p - 1)! ≡ -1 [MOD p] := by
  /- 使用Nat版本的威尔逊定理 -/
...
-- 威尔逊定理的逆：若 (n-1)! ≡ -1 (mod n)，则 n 是素数
theorem wilson_converse {n : ℕ} (hn : n > 1) (h : (n - 1)! ≡ -1 [MOD n]) :
    n.Prime := by
  /- 威尔逊定理的逆也是成立的 -/
  exact Nat.prime_of_fac_equiv_neg_one hn h

-- 威尔逊定理的等价形式：n 是素数 ⟺ (n-1)! ≡ -1 (mod n)
theorem wilson_iff_prime {n : ℕ} (hn : n > 1) :
    n.Prime ↔ (n - 1)! ≡ -1 [MOD n] := by
  constructor
  · intro h_prime
    exact wilson_theorem_nat h_prime
  · intro h
    exact wilson_converse hn h
...
-- 验证 p = 5
example : (5 - 1)! ≡ -1 [MOD 5] := by
  norm_num [Nat.ModEq]

-- 验证 p = 7
example : (7 - 1)! ≡ -1 [MOD 7] := by
  norm_num [Nat.ModEq]

-- 验证 p = 11
example : (11 - 1)! ≡ -1 [MOD 11] := by
  norm_num [Nat.ModEq]
...
-- 5 是威尔逊素数
example : IsWilsonPrime 5 := by
  constructor
  · norm_num
  · norm_num [Nat.factorial]

-- 13 是威尔逊素数
example : IsWilsonPrime 13 := by
  constructor
  · norm_num
  · norm_num [Nat.factorial]
