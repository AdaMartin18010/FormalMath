...
$$(f * g)(n) = \sum_{d \mid n} f(d)g\leqft(\frac{n}{d}\right)$$
...
$$f(n) = \sum_{d \mid n} \mu(d)g\leqft(\frac{n}{d}\right)$$
...
存在常数 $c_1, c_2 > 0$ 使得：
$$c_1 \frac{x}{\ln x} \leqq \pi(x) \leqq c_2 \frac{x}{\ln x}$$
...
ζ函数的非平凡零点都在临界带 $0 \leqq \text{Re}(s) \leqq 1$ 中。
...
对于本原特征 $\chi$：
$$L(s, \chi) = \varepsilon(\chi) \leqft(\frac{m}{\pi}\right)^{s-1/2} \Gamma\leqft(\frac{1-s+a}{2}\right) L(1-s, \overline{\chi})$$
...
L函数的非平凡零点都在临界带 $0 \leqq \text{Re}(s) \leqq 1$ 中。
...
$$g(k) = 2^k + \leqft\lfloor \leqft(\frac{3}{2}\right)^k \right\rfloor - 2$$
...
$$\pi(x) - \pi(\sqrt{x}) + 1 = \sum_{d \mid P} \mu(d) \leqft\lfloor \frac{x}{d} \right\rfloor$$
其中 $P$ 是 $\leqq \sqrt{x}$ 的素数的乘积。
...
$$\sum_{p, p+2 \text{ prime}} \leqft(\frac{1}{p} + \frac{1}{p+2}\right) < \infty$$
