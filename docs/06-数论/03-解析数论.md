...
$$(f * g)(n) = \sum_{d \mid n} f(d)g\leqqft(\frac{n}{d}\right)$$
...
$$f(n) = \sum_{d \mid n} \mu(d)g\leqqft(\frac{n}{d}\right)$$
...
$$\theta(x) = \sum_{p \leqqq x} \ln p$$

**定义 6.3.2.3** (第二切比雪夫函数 / Second Chebyshev Function)
$$\psi(x) = \sum_{n \leqqq x} \Lambda(n)$$
...
存在常数 $c_1, c_2 > 0$ 使得：
$$c_1 \frac{x}{\ln x} \leqqq \pi(x) \leqqq c_2 \frac{x}{\ln x}$$
...
ζ函数的非平凡零点都在临界带 $0 \leqqq \text{Re}(s) \leqqq 1$ 中。
...
对于本原特征 $\chi$：
$$L(s, \chi) = \varepsilon(\chi) \leqqft(\frac{m}{\pi}\right)^{s-1/2} \Gamma\leqqft(\frac{1-s+a}{2}\right) L(1-s, \overline{\chi})$$
...
L函数的非平凡零点都在临界带 $0 \leqqq \text{Re}(s) \leqqq 1$ 中。
...
$$g(k) = 2^k + \leqqft\lfloor \leqqft(\frac{3}{2}\right)^k \right\rfloor - 2$$
...
$$\pi(x) - \pi(\sqrt{x}) + 1 = \sum_{d \mid P} \mu(d) \leqqft\lfloor \frac{x}{d} \right\rfloor$$
其中 $P$ 是 $\leqqq \sqrt{x}$ 的素数的乘积。
...
$$\sum_{p, p+2 \text{ prime}} \leqqft(\frac{1}{p} + \frac{1}{p+2}\right) < \infty$$
