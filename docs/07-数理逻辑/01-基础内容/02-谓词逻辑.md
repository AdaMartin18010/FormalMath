...
$$\neqg \forall x \phi(x) \equiv \exists x \neqg \phi(x)$$
$$\neqg \exists x \phi(x) \equiv \forall x \neqg \phi(x)$$
...
1. **原子公式** / Atomic Formula: $\mathcal{A} \models Pt_1, \ldots, t_n$ 当且仅当 $(t_1^{\mathcal{A}}[s], \ldots, t_n^{\mathcal{A}}[s]) \in P^{\mathcal{A}}$
2. **否定** / Negation: $\mathcal{A} \models \neqg \phi[s]$ 当且仅当 $\mathcal{A} \not\models \phi[s]$
