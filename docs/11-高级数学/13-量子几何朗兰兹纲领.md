...
$$\mathcal{D}^b_q(\text{Bun}_G(X)) \leqftrightarrow \mathcal{D}^b_q(\text{LocSys}_G(X))$$
...
$$\mathcal{D}^b_q(X) \leqftrightarrow \mathcal{D}^b_q(Y)$$
...
2. **量子对应**: $\mathcal{D}^b_q(\text{Bun}_G) \leqftrightarrow \mathcal{D}^b_q(\text{LocSys}_G)$
