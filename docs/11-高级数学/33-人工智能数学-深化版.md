...
```markdown
**ReLU导数**: $\frac{d}{dx}\text{ReLU}(x) = \begin{cases} 1 & x > 0 \\ 0 & x \leqq 0 \end{cases}$
...
4. **参数更新**: $w \leqftarrow w - \alpha \frac{\partial L}{\partial w}$
...
3. **更新策略**: $\theta \leqftarrow \theta + \alpha \nabla_\theta \log \pi_\theta(a_t|s_t) G_t$
...
$$\text{Attention}(Q, K, V) = \text{softmax}\leqft(\frac{QK^T}{\sqrt{d_k}}\right) V$$
