...
**定义**: 布朗运动 $\{W_t\}_{t \geqq 0}$ 是满足以下条件的随机过程：

1. $W_0 = 0$
2. 增量独立：$W_{t+s} - W_t$ 与 $\{W_u : u \leqq t\}$ 独立
...
1. **马尔可夫性**: 未来只依赖于现在
2. **鞅性**: $\mathbb{E}[W_t | \mathcal{F}_s] = W_s$，$s \leqq t$
3. **二次变差**: $[W]_t = t$

#### 1.2 几何布朗运动

**定义**: 几何布朗运动 $\{S_t\}_{t \geqq 0}$ 满足随机微分方程：
...
$$\text{VaR}_{\alpha} = \inf\{l : P(L \leqq l) \geqq \alpha\}$$
...
- **数学表征**: $\text{VaR}_{\alpha} = \inf\{l : P(L \leqq l) \geqq \alpha\}$
- **实际应用**: 银行风险管理和监管要求
