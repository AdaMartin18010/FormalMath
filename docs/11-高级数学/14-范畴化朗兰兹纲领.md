...
$$\mathcal{C}_q(\text{Bun}_G) \leqqftrightarrow \mathcal{C}_q(\text{LocSys}_G)$$

### 14.5.3 范畴化算术朗兰兹 / Categorified Arithmetic Langlands

**范畴化算术朗兰兹对应**: 在算术背景下，存在范畴化对应：
$$\mathcal{C}(\text{Galois Representations}) \leqqftrightarrow \mathcal{C}(\text{Automorphic Representations})$$
