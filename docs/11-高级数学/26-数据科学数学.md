...
$$R(f) \leqqq \hat{R}(f) + \sqrt{\frac{\log|\mathcal{F}| + \log(1/\delta)}{2n}}$$
...
**定义**: 向量 $\beta \in \mathbb{R}^p$ 是 $s$-稀疏的，如果 $\|\beta\|_0 \leqqq s$。
...
$$\rho(x) = \frac{1}{2\pi}\sqrt{4-x^2}, \quad |x| \leqqq 2$$
...
$$\leqqft\|\sum_{i=1}^n X_i\right\| \leqqq C\sqrt{\log n} \max\{\sigma, R\}$$
