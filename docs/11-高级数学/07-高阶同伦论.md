...
$$\pi_n(X, x_0) = [S^n, X]_{x_0}, \quad n \geqq 2$$
...
**定义**: 设 $X$ 是拓扑空间，$x_0 \in X$ 是基点，$n \geqq 1$，$n$ 阶同伦群定义为：
...
1. **阿贝尔性**: $\pi_n(X, x_0)$ 是阿贝尔群（$n \geqq 2$）
...
1. 对于每个 $(p,q)$，存在 $r_0$ 使得 $E_r^{p,q} = E_{r_0}^{p,q}$ 对所有 $r \geqq r_0$
...
$$\Sigma^{\infty}X = \{\Sigma^n X\}_{n \geqq 0}$$
...
**同伦纤维积**: 设 $X \to Z \leqftarrow Y$ 是映射，同伦纤维积定义为：
...
**定义**: A∞代数 $A$ 是向量空间 $A$ 和映射 $\{m_n: A^{\otimes n} \to A\}_{n \geqq 1}$，满足：
...
1. **高阶同伦群**: $\pi_n(X, x_0) = [S^n, X]_{x_0}$ 是阿贝尔群（$n \geqq 2$）
