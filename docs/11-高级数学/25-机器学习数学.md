...
**导数**: $\frac{d}{dx}\text{ReLU}(x) = \begin{cases} 1 & \text{if } x > 0 \\ 0 & \text{if } x \leqqq 0 \end{cases}$
...
**定义**:
$$I_{ij}(\theta) = \mathbb{E}\leqqft[\frac{\partial \log p(x|\theta)}{\partial \theta_i} \frac{\partial \log p(x|\theta)}{\partial \theta_j}\right]$$
