...
**导数**: $\frac{d}{dx}\text{ReLU}(x) = \begin{cases} 1 & \text{if } x > 0 \\ 0 & \text{if } x \leqq 0 \end{cases}$
...
**定义**:
$$I_{ij}(\theta) = \mathbb{E}\leqft[\frac{\partial \log p(x|\theta)}{\partial \theta_i} \frac{\partial \log p(x|\theta)}{\partial \theta_j}\right]$$
