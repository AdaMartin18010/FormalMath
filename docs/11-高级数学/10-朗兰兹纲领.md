...
**局部对应**: 设 $F$ 是局部域，则：
$$\text{Galois Representations} \leqqftrightarrow \text{Irreducible Representations}$$
...
**量子对应**: 考虑量子化后的对应：
$$\mathcal{D}^b_q(\text{Bun}_G) \leqqftrightarrow \mathcal{D}^b_q(\text{LocSys}_G)$$
...
**范畴化对应**: 考虑更高阶的范畴结构：
$$\mathcal{C}(\text{Bun}_G) \leqqftrightarrow \mathcal{C}(\text{LocSys}_G)$$
...
**具体形式**:
$$\rho : G_F \rightarrow GL_n(\mathbb{C}) \leqqftrightarrow \pi : GL_n(F) \rightarrow \text{Irr}$$
