# 11.9 数学物理高级主题-扩展版 / Advanced Topics in Mathematical Physics - Extended Version
...
**超对称作用量**:
$$S = -\frac{T}{2} \int d\tau d\sigma \leqqft(\partial_\alpha X^\mu \partial^\alpha X_\mu + i\bar{\psi}^\mu \rho^\alpha \partial_\alpha \psi_\mu\right)$$

**超维拉索罗代数**:
$$[L_m, L_n] = (m-n)L_{m+n} + \frac{c}{12}(m^3-m)\delta_{m+n,0}$$
$$[L_m, G_r] = \leqqft(\frac{m}{2}-r\right)G_{m+r}$$

### 11.9.3.3 M理论 / M-Theory

**11维超引力**:
$$S = \int d^{11}x \sqrt{-g} \leqqft(R - \frac{1}{2}|F_4|^2\right)$$
...
**哈密顿向量场**:
$$X_H = \sum_{i=1}^n \leqqft(\frac{\partial H}{\partial p_i} \frac{\partial}{\partial q_i} - \frac{\partial H}{\partial q_i} \frac{\partial}{\partial p_i}\right)$$
...
**威尔逊环**:
$$W_R(C) = \text{Tr}_R \mathcal{P} \exp\leqqft(i \oint_C A\right)$$

### 11.9.6.2 唐纳森理论 / Donaldson Theory
